...
{
    /// <summary>
    /// Editor window for testing voxel explosions and craters.
    /// Provides interactive crater creation in Scene View.
    /// </summary>
    public class VoxelExplosionTesterWindow : EditorWindow
    {
        // Crater settings
        ...
        [MenuItem("DIG/Voxel/Explosion Tester")]
        public static void ShowWindow()
        {
            var window = GetWindow<VoxelExplosionTesterWindow>("Explosion Tester");
        ...
        private void OnGUI()
        {
            ...
            EditorGUILayout.LabelField("Voxel Explosion Tester", EditorStyles.boldLabel);
            
            if (!Application.isPlaying)
            {
                EditorGUILayout.HelpBox("Enter Play Mode to test explosions.", MessageType.Info);
                DrawSettingsSection();
        ...
        private void HandlePlacementMode(SceneView sceneView)
        {
            ...
            // Only repaint scene view on mouse events, not every frame
        ...
        {
            // Find ServerWorld to act authoritatively
            ...
            if (targetWorld == null)
            {
                UnityEngine.Debug.LogError("[ExplosionTester] No Server world found! Cannot spawn authoritative explosion.");
                return;
            }
            ...
            UnityEngine.Debug.Log($"[ExplosionTester] Queued authoritative explosion in {targetWorld.Name}");
            
            // Update stats
            _totalExplosions++;
            _lastExplosionTime = Time.time;
            
            // Estimate voxels (actual count comes from system)
            float volume = (4f / 3f) * Mathf.PI * _radius * _radius * _radius;
