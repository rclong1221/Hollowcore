...
{
    /// <summary>
    /// EPIC 17.3: Receives TradeAcceptRpc from target player, transitions Pending→Active.
    /// </summary>
    [WorldSystemFilter(WorldSystemFilterFlags.ServerSimulation)]
    [UpdateInGroup(typeof(SimulationSystemGroup))]
    [UpdateAfter(typeof(TradeCancelReceiveSystem))]
    public partial class TradeAcceptReceiveSystem : SystemBase
    {
        ...
        protected override void OnCreate()
        {
            _rpcQuery = GetEntityQuery(
                ComponentType.ReadOnly<TradeAcceptRpc>(),
        ...
        protected override void OnUpdate()
        {
            ...
            for (int i = 0; i < entities.Length; i++)
            {
                ...
                // Find pending session where this player is the target
                foreach (var (state, sessionEntity) in
                         SystemAPI.Query<RefRW<TradeSessionState>>()
                             .WithAll<TradeSessionTag>()
                             .WithEntityAccess())
                {
                    if (state.ValueRO.State != TradeState.Pending) continue;
                    if (state.ValueRO.TargetEntity != player) continue;

                    state.ValueRW.State = TradeState.Active;

                    // Notify both players
                    var notifyInit = ecb.CreateEntity();
                    ecb.AddComponent(notifyInit, new TradeStateNotifyRpc { NewState = TradeState.Active, FailReason = 0 });
                    ...
                    ecb.AddComponent(notifyTarget, new TradeStateNotifyRpc { NewState = TradeState.Active, FailReason = 0 });
                    ...
                    {
                        Type = TradeVisualEventType.SessionStarted,
                        Payload = 0
                    });
                    break;
                }
