...
namespace Opsive.UltimateCharacterController.AddOns.Agility
{
    ...
    public class LedgeStrafe : DetectGroundAbilityBase
    {
        ...
        [Tooltip("The amount of time after the ability has stopped before it can be started again.")]
        [SerializeField] protected float m_StopCooldown = 0.2f;
        ...
        public override bool CanStartAbility()
        {
            ...
            if (m_StopTime + m_StopCooldown >= Time.time) {
                return false;
            }
        ...
        {
            if (!active || m_StopTime != Time.time|| !(ability is Jump)) {
                return;
            }
