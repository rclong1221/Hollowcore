...
namespace Opsive.Shared.StateSystem
{
    ...
    public class StateManager : MonoBehaviour
    {
        ...
        private static StateManager Instance {
            get {
                if (!s_Initialized) {
                    ...
                    s_Instance = UnityEngine.Object.FindAnyObjectByType<StateManager>();
#else
                    s_Instance = UnityEngine.Object.FindObjectOfType<StateManager>();
#endif
                    if (s_Instance == null) {
        ...
        private Dictionary<GameObject, List<string>> m_ActiveCharacterStates = new Dictionary<GameObject, List<string>>();
        ...
        {
            // The state may have already been initialized
            if (states[states.Length - 1]?.Preset != null) {
                return;
            }
            ...
            if (characterGameObject != null) {
                if (characterGameObject == stateGameObject) {
                    ...
                    if (!m_ActiveCharacterStates.ContainsKey(stateGameObject)) {
                        m_ActiveCharacterStates.Add(stateGameObject, new List<string>());
                    }
                } else {
                    ...
                    if (m_ActiveCharacterStates.TryGetValue(characterGameObject, out var activeStates)) {
                        for (int i = activeStates.Count - 1; i >= 0; --i) {
                            SetState(stateGameObject, activeStates[i], true);
                        }
                    }
                }
            }
            ...
            if (linkedGameObjectList != null) {
                if (link) {
                    ...
                    if (m_ActiveCharacterStates.TryGetValue(original, out var activeStates)) {
                        for (int i = activeStates.Count - 1; i >= 0; --i) {
                            SetState(linkedGameObject, activeStates[i], true);
                        }
                    }
                } else {
