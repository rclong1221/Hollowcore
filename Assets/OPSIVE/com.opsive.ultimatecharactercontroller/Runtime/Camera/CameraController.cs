...
namespace Opsive.UltimateCharacterController.Camera
{
    ...
    public class CameraController : StateBehavior, ILookSource, Shared.Camera.ICamera
    {
        ...
        [SerializeReference] protected ViewType[] m_ViewTypes;
        ...
        protected ViewType m_ActiveViewType;
        ...
        public GameObject GameObject { get { return m_ActiveViewType.GameObject; } }
        public Transform Transform { get { return m_ActiveViewType.Transform; } }
        public Transform CharacterTransform { get { return m_CharacterTransform; } }
        public Rigidbody CharacterRigidbody { get { return m_CharacterRigidbody; } }
        ...
        public virtual ViewType[] ViewTypes { get { return m_ViewTypes; }
            set
            {
                m_ViewTypes = value;
        ...
        public ViewType ActiveViewType { get { return m_ActiveViewType; } }
        public string FirstPersonViewTypeFullName { get { return m_FirstPersonViewTypeFullName; }
            set
            {
                ...
                if (Application.isPlaying) {
                    if (m_ActiveViewType != null && m_ActiveViewType.FirstPersonPerspective) {
        ...
        public float LookDirectionDistance { get { return m_ActiveViewType.LookDirectionDistance; } }
        public float Pitch { get { return m_ActiveViewType.Pitch; } }
        public bool UseSmoothOffset { get { return m_ActiveViewType.UseSmoothOffset; } }
        public bool Zoom { get { return m_Zoom; } }
        public bool IsTransitioning { get { return m_Transitioner != null && m_Transitioner.IsTransitioning; } }
        ...
        protected override void Awake()
        {
            ...
            m_ActiveViewType = null; // The ViewType may have been assigned during fast enter playmode.
            InitializeViewTypes();
            if (m_ActiveViewType == null) {
                Debug.LogError("Error: The Active ViewType is null. Ensure a ViewType is selected.", this);
                return;
            }
        ...
        /// <summary>
        /// Initialize the view types.
        /// </summary>
        public void InitializeViewTypes()
        {
            if (m_ViewTypes == null || m_ViewTypes.Length == 0) {
                return;
            }

            // Do a sanity check to ensure none of the view types are null.
            Utility.UnityEngineUtility.RemoveNullElements(ref m_ViewTypes);
        ...
        {
            if ((m_ActiveViewType != null && m_ActiveViewType.GetType() == type) || type == null) {
                return;
            }

            // The ViewTypes may not be initialized yet.
            ...
            var baseCharacterRotation = Quaternion.identity;
            // ViewType will be null on startup.
            if (m_ActiveViewType != null && m_Character != null && Application.isPlaying) {
                pitch = m_ActiveViewType.Pitch;
                yaw = m_ActiveViewType.Yaw;
                baseCharacterRotation = m_ActiveViewType.BaseCharacterRotation;
                m_ActiveViewType.ChangeViewType(false, 0, yaw, baseCharacterRotation);

                EventHandler.ExecuteEvent(m_GameObject, "OnCameraChangeViewTypes", m_ActiveViewType, false);
                if (m_OnChangeViewTypesEvent != null) {
                    m_OnChangeViewTypesEvent.Invoke(m_ActiveViewType, false);
                }
            }

            var originalViewType = m_ActiveViewType;
            m_ViewTypeFullName = type.FullName;
            m_ActiveViewType = m_ViewTypes[index];

            // Keep the first/third person view type updated to be able to switch back to the last active type.
            if (m_ActiveViewType.FirstPersonPerspective) {
                m_FirstPersonViewTypeFullName = m_ViewTypeFullName;
                m_FirstPersonViewType = m_ActiveViewType;
            } else {
                m_ThirdPersonViewTypeFullName = m_ViewTypeFullName;
                m_ThirdPersonViewType = m_ActiveViewType;
            }
            ...
            if (originalViewType != null && m_Character != null && Application.isPlaying) {
                m_ActiveViewType.ChangeViewType(true, pitch, yaw, baseCharacterRotation);

                EventHandler.ExecuteEvent(m_GameObject, "OnCameraChangeViewTypes", m_ActiveViewType, true);
                if (m_OnChangeViewTypesEvent != null) {
                    m_OnChangeViewTypesEvent.Invoke(m_ActiveViewType, true);
                }
                if (originalViewType.FirstPersonPerspective != m_ActiveViewType.FirstPersonPerspective) {
                    EventHandler.ExecuteEvent<bool>(m_Character, "OnCameraWillChangePerspectives", m_ActiveViewType.FirstPersonPerspective);
                }
                ...
                if (!immediateTransition && m_Transitioner != null) {
                    ...
                    if (m_Transitioner.StartTransition(originalViewType, m_ActiveViewType)) {
                        return;
                    ...
                    m_Transform.rotation = m_ActiveViewType.Rotate(0, 0, true);
                    m_Transform.position = m_ActiveViewType.Move(true);
                    SimulationManager.SetCameraRotation(m_SimulationIndex, m_Transform.rotation);
                    SimulationManager.SetCameraPosition(m_SimulationIndex, m_Transform.position);
                }
                ...
                if (originalViewType.FirstPersonPerspective != m_ActiveViewType.FirstPersonPerspective) {
                    EventHandler.ExecuteEvent<bool>(m_Character, "OnCameraChangePerspectives", m_ActiveViewType.FirstPersonPerspective);

                    if (m_OnChangePerspectivesEvent != null) {
                        m_OnChangePerspectivesEvent.Invoke(m_ActiveViewType.FirstPersonPerspective);
                    }
                }
            }
        }
        ...
        {
            m_SimulationIndex = SimulationManager.RegisterCamera(this);
        }
        ...
        protected virtual void InitializeCharacter(GameObject character)
        {
            ...
            if (m_Character != null) {
                ...
                EventHandler.UnregisterEvent(m_Character, "OnAnimatorWillSnap", PositionImmediately);
            ...
            if (character != null) {
                ...
                m_CharacterRigidbody = character.GetCachedComponent<Rigidbody>();
            ...
            enabled = m_CharacterLocomotion != null;
            ...
            if (m_Character != null) {
                m_ActiveViewType.ChangeViewType(true, 0, 0, m_CharacterRigidbody.rotation);
                m_ActiveViewType.UpdateFieldOfView(true);
                m_Transform.rotation = m_ActiveViewType.Rotate(0, 0, true);
                m_Transform.position = m_ActiveViewType.Move(true);
                ...
                EventHandler.RegisterEvent(m_Character, "OnAnimatorWillSnap", PositionImmediately);
                ...
                var recommendedMovementTypes = m_ActiveViewType.GetType().GetCustomAttributes(typeof(RecommendedMovementType), true);
        ...
        private void InitializeAnchor()
        {
            ...
            if (m_AutoAnchor) {
                ...
                if (modelManager != null) {
                    ...
                    var animationMonitor = m_Character.GetComponentInChildren<AnimationMonitorBase>();
                    if (animationMonitor != null) {
                        animator = animationMonitor.gameObject.GetCachedComponent<Animator>();
                    }
                }
                if (animator != null && (anchor = animator.GetBoneTransform(m_AutoAnchorBone)) != null) {
                    m_Anchor = anchor;
                }
            }
            
            if (m_Anchor != null && !m_Anchor.IsChildOf(m_Character.transform)) {
                m_Anchor = null;
            }

            if (m_Anchor == null) {
                m_Anchor = m_CharacterRigidbody.transform;
            }
        }
        ...
        public virtual void Rotate(float horizontalMovement, float verticalMovement)
        {
            ...
            if (m_AdjustWithTimescale) {
                horizontalMovement *= m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale;
                verticalMovement *= m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale;
            }
        ...
        public virtual void Move()
        {
            ...
            m_ActiveViewType.UpdateFieldOfView(false);
            ...
            m_Transform.position = m_ActiveViewType.Move(false);
        }
        ...
        /// <summary>
        /// Tries to start or stop the camera zoom.
        /// </summary>
        ...
        public Quaternion GetCrosshairsDeltaRotation() { return m_ActiveViewType.GetCrosshairsDeltaRotation(); }
        ...
        public virtual void TogglePerspective()
        {
            ...
            SetPerspective(!m_ActiveViewType.FirstPersonPerspective);
        }
#endif
        ...
        public Vector3 LookPosition(bool characterLookPosition)
        {
            ...
            return m_ActiveViewType.LookPosition(characterLookPosition);
        }
        ...
        public Vector3 LookDirection(bool characterLookDirection)
        {
            ...
            return m_ActiveViewType.LookDirection(characterLookDirection);
        }
        ...
        public Vector3 LookDirection(Vector3 lookPosition, bool characterLookDirection, int layerMask, bool includeRecoil, bool includeMovementSpread)
        {
            ...
            return m_ActiveViewType.LookDirection(lookPosition, characterLookDirection, layerMask, includeRecoil, includeMovementSpread);
        }
        ...
        public virtual void PositionImmediately(bool resetViewTypes)
        {
            ...
            if (resetViewTypes) {
                ...
                for (int i = 0; i < m_ViewTypes.Length; ++i) {
                    m_ViewTypes[i].Reset(m_CharacterRigidbody.rotation);
                }
            }
            ...
            EventHandler.ExecuteEvent(m_Character, "OnCameraImmediatePosition");
        }
        ...
        public void SetRotationalOverride(System.Func<Vector3, Quaternion, Quaternion> overrideFunc)
        { 
            for (int i = 0; i < m_ViewTypes.Length; ++i) {
                m_ViewTypes[i].SetRotationalOverride(overrideFunc);
            }
        }
        ...
        {
            SimulationManager.UnregisterCamera(m_SimulationIndex);
        }
