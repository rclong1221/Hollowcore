...
namespace Opsive.UltimateCharacterController.Objects
{
    ...
    /// </summary>
    public class TrajectoryObject : MonoBehaviour, IForceObject, IDamageSource, ISmoothedObject
    {
        ...
        [Tooltip("When the velocity has a square magnitude value less than the specified value the object will be considered settled.")]
        [SerializeField] protected float m_SettlePositionThreshold = 0.01f;
        [Tooltip("When the torque has a square magnitude value less than the specified value the object will be considered settled.")]
        [SerializeField] protected float m_SettleRotationThreshold = 0.01f;
        ...
        public float SettlePositionThreshold { get { return m_SettlePositionThreshold; } set { m_SettlePositionThreshold = Mathf.Max(value, 0.001f); } }
        public float SettleRotationThreshold { get { return m_SettleRotationThreshold; } set { m_SettleRotationThreshold = Mathf.Max(value, 0.001f); } }
        ...
        protected virtual void Awake()
        {
            ...
            // The settle threshold has to be positive.
            if (m_SettlePositionThreshold <= 0) { m_SettlePositionThreshold = 0.001f; }
            if (m_SettleRotationThreshold <= 0) { m_SettleRotationThreshold = 0.001f; }
        ...
        private bool Move(ref Vector3 position, Quaternion rotation, bool simulation)
        {
            ...
            if (m_MovementSettled && (simulation || m_RotationSettled) && m_SettlePositionThreshold > 0) {
                return true;
            }

            // Stop moving if the velocity is less than a minimum threshold and the object is on the ground.
            if (m_Velocity.sqrMagnitude < m_SettlePositionThreshold && (simulation || m_RotationSettled)) {
            ...
            var deltaTime = m_TimeScale * Time.fixedDeltaTime * TimeUtility.TimeScale;
            ...
            if (Cast(position, rotation, direction)) {
                ...
                if (m_CollisionMode == CollisionMode.Collide) {
                    ...
                    var dynamicFrictionValue = m_Collider != null ? Mathf.Clamp01(1 - MathUtility.FrictionValue(m_Collider.material, m_RaycastHit.collider.material, true)) : (simulation ? m_RaycastHit.collider.material.dynamicFriction : 0);
        ...
        private void Rotate(Vector3 position, ref Quaternion rotation)
        {
            ...
            if ((m_CollisionMode == CollisionMode.Collide || m_Collided) && (m_Torque.sqrMagnitude < m_SettlePositionThreshold || m_MovementSettled)) {
                if (m_Collider is CapsuleCollider || m_Collider is BoxCollider) {
                    if (!m_RotationSettled) {
                        ...
                        if (dot > m_SettleRotationThreshold && dot < (1f - m_SettleRotationThreshold) || m_Velocity.sqrMagnitude > m_SettlePositionThreshold) {
