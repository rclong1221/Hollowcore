...
namespace Opsive.UltimateCharacterController.Objects
{
    ...
    public class MovingPlatform : StateBehavior, IInteractableTarget, ISmoothedObject
    {
        /// <summary>
        ...
        public struct Waypoint
        {
            ...
            [SerializeField] [StateName] private string m_State;

            private long m_StateHash;
            ...
            public long StateHash { get { return m_StateHash; } }
        ...
        public enum PathMovementType
        {
            ...
            Random      // Moves to a random waypoint.
        }
        ...
        [Tooltip("If using the Random PathMovementType, specifies the seed that should be used for the random number generator. Specify -1 to not use a seed.")]
        [SerializeField] protected int m_RandomSeed;
        ...
        public PathMovementType MovementType { get { return m_MovementType; }
            set
            {
                m_MovementType = value;
                // The random movement type can initialize a seed.
                if (m_MovementType == PathMovementType.Random && m_RandomSeed != -1) {
        ...
        [NonSerialized] public int SimulationIndex { get { return m_SimulationIndex; } set { m_SimulationIndex = value; } }
        ...
        {
            if (m_MovementType == PathMovementType.Random) {
                return Random.Range(0, m_Waypoints.Length);
            }
            return Mathf.Max((m_NextWaypoint + (increase ? 1 : -1)) % m_Waypoints.Length, 0);
        }
        ...
        {
            m_Rigidbody.rotation = m_Transform.rotation = m_MoveRotation;
        }
        ...
        {
            m_Rigidbody.position = m_Transform.position = m_MovePosition;

            // Progress the move time and also store the updated metrics.
            m_MoveTime += m_MovementSpeed * 0.01f * TimeUtility.DeltaTime;
        }
        ...
        public bool CanInteract(GameObject character, Character.Abilities.Interact interactAbility)
        {
            return true;
        }
        ...
        public void Interact(GameObject character, Character.Abilities.Interact interactAbility)
        {
            if (m_EnableOnInteract) {
                enabled = true;
