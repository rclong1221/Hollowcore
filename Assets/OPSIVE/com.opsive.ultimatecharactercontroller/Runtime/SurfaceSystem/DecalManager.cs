    ...
    public class DecalManager : MonoBehaviour
    {
        ...
        private static DecalManager Instance
        {
            get
            {
                if (!s_Initialized) {
                    ...
                    s_Instance = UnityEngine.Object.FindAnyObjectByType<DecalManager>();
#else
                    s_Instance = UnityEngine.Object.FindObjectOfType<DecalManager>();
#endif
                    if (s_Instance == null) {
        ...
        private void Update()
        {
            for (int i = m_DecalsToFade.Count - 1; i >= 0; --i) {
                ...
                color.a = Mathf.Lerp(color.a, 0, TimeUtility.DeltaTime * m_RemoveFadeoutSpeed);
                // The decal can be removed from the list when it is completely faded out.
                if (color.a == 0) {
                    if (ObjectPoolBase.IsPooledObject(m_DecalsToFade[i].gameObject)) {
