...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Character
{
    ...
    public class PerspectiveMonitor : StateBehavior
    {
        ...
        private void OnAttachLookSource(ILookSource lookSource)
        {
            ...
            m_MaterialSwapper = lookSource.GameObject.GetCachedComponent<MaterialSwapper>();
        }
#endif
        ...
        private void StartInternal()
        {
            ...
            if (networkInfo != null && !networkInfo.IsLocalPlayer() && !networkInfo.HasAuthority() && !networkInfo.IsSpectator()) {
...
#if FIRST_PERSON_CONTROLLER
            // The MaterialWapper will manage the swapping.
            if (m_MaterialSwapper == null || !m_MaterialSwapper.enabled) {
#endif
        ...
        {
#if ULTIMATE_CHARACTER_CONTROLLER_MULTIPLAYER
            var firstPersonPerspective = (m_CharacterLocomotion.NetworkInfo == null || m_CharacterLocomotion.NetworkInfo.IsLocalPlayer() || m_CharacterLocomotion.NetworkInfo.IsSpectator()) ? m_CharacterLocomotion.FirstPersonPerspective : false;
#else
            ...
            for (int i = 0; i < m_Renderers.Count; ++i) {
                ...
                if (m_DeathVisibility == ObjectDeathVisiblity.AllInvisible) {
                    invisibleObject = firstPersonPerspective;
...
#endif
            for (int i = 0; i < m_Renderers.Count; ++i) {
                m_Renderers[i].materials = (!firstPersonPerspective || 
