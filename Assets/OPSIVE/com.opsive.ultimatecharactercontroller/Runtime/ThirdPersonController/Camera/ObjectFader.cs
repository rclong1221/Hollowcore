...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Camera
{
    ...
    public class ObjectFader : StateBehavior
    {
        ...
        [Tooltip("The name of the material property that switches between opaque and transparent settings.")]
        [SerializeField] protected string m_ModePropertyName = "_Mode";
        [Tooltip("The value of the mode that indicates transparency is active.")]
        [SerializeField] protected int m_ModeTransparentValue = 2;
        ...
        public string ModePropertyName { get { return m_ModePropertyName; } set { m_ModePropertyName = value; } }
        public int ModeTransparentValue { get { return m_ModeTransparentValue; } set { m_ModeTransparentValue = value; } }
        ...
        private PerspectiveMonitor m_CharacterPerspectiveMonitor;
        ...
        protected override void Awake()
        {
            ...
            m_ModeID = Shader.PropertyToID(m_ModePropertyName);
        ...
        protected virtual void OnAttachCharacter(GameObject character)
        {
            ...
            if (m_Character != null && m_Character != character) {
                ...
                m_CharacterPerspectiveMonitor = null;
            }
            ...
            if (m_Character != null) {
                ...
                m_CharacterPerspectiveMonitor = m_Character.GetCachedComponent<PerspectiveMonitor>();
                if (m_CharacterFade) {
                    ...
                    if (count > 0) {
                        ...
                        for (int i = 0; i < renderers.Length; ++i) {
                            ...
                            for (int j = 0; j < materials.Length; ++j) {
                                ...
                                if (materials[j].HasProperty(m_ColorID)) {
                                    if (materials[j].HasProperty(m_ModeID)) {
                                    ...
                                    if (m_CacheCharacterMaterials) {
                                        ...
                                        originalMaterialValues.Initialize(materials[j], m_ColorID, m_MaterialModeSet.Contains(materials[j]) ? m_ModeID : 0);
        ...
        {
            if (!m_CharacterFade || TimeUtility.Time < m_CharacterFadeCooldownElapsedTime) {
                return;
            }
        ...
        public void MultiCameraRender(bool start)
        {
            ...
            if (m_CharacterFade && !m_CameraController.IsTransitioning) {
                m_FadeOffset = MathUtility.InverseTransformPoint(m_CharacterTransform.TransformPoint(m_TransformOffset), m_CharacterTransform.rotation, m_Transform.position);
        ...
        private void EnableFadeMaterial(Material material)
        {
            ...
            if (!m_CacheCharacterMaterials && !m_OriginalMaterialValuesMap.ContainsKey(material)) {
                var originalMaterialValues = GenericObjectPool.Get<OriginalMaterialValue>();
                originalMaterialValues.Initialize(material, m_ColorID, m_MaterialModeSet.Contains(material) ? m_ModeID : 0);
                m_OriginalMaterialValuesMap.Add(material, originalMaterialValues);
            }

            if (m_MaterialModeSet.Contains(material)) {
                material.SetFloat(m_ModeID, m_ModeTransparentValue);
        ...
        private void RevertMaterial(Material material, OriginalMaterialValue originalMaterialValue)
        {
            ...
            if (originalMaterialValue.ContainsMode) {
                material.SetFloat(m_ModeID, originalMaterialValue.Mode);
                        ...
                        for (int k = 0; k < materials.Length; ++k) {
                            ...
                            if (canFade) {
                                ...
                                if (!m_OriginalMaterialValuesMap.ContainsKey(material)) {
                                    ...
                                    if (m_AutoSetMode && material.HasProperty(m_ModeID)) {
        ...
        public bool CanMaterialFade(Material material)
        {
            ...
            return m_DisableCollider && material.HasProperty(m_ModeID) && (m_AutoSetMode || material.GetInt(m_ModeID) == m_ModeTransparentValue);
        }
        ...
        private void OnAddItem(CharacterItem characterItem)
        {
            ...
            if (m_CharacterFade) {
                ...
                if (m_CharacterPerspectiveMonitor != null) {
                    m_CharacterPerspectiveMonitor.UpdateThirdPersonMaterials(true);
                }
                InitializeCharacterFadeRenderers(perspectiveItem.Object.GetComponentsInChildren<Renderer>(true));
                if (m_CharacterPerspectiveMonitor != null) {
                    m_CharacterPerspectiveMonitor.UpdateThirdPersonMaterials(false);
                }
            }
        }
        ...
        public void UpdateMaterials(Material previousMaterial, Material targetMaterial)
        {
            ...
            if (targetMaterial.HasProperty(m_ModeID)) {
            ...
            if (m_CacheCharacterMaterials) {
                var originalMaterialValues = GenericObjectPool.Get<OriginalMaterialValue>();
                originalMaterialValues.Initialize(targetMaterial, m_ColorID, m_ModeID);
        ...
        public override void StateChange()
        {
            ...
            if (m_CharacterFadeStateChangeCooldown > 0) {
                m_CharacterFadeCooldownElapsedTime = TimeUtility.Time + m_CharacterFadeStateChangeCooldown;
            }
