...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Camera.ViewTypes
{
    ...
    public class RPG : ThirdPerson
    {
        ...
        [Tooltip("The name of the input mapping which will force the character to look in the same direction as the camera.")]
        [SerializeField] protected string m_CharacterForcedRotationInputName = "Fire2";
        ...
        public string CharacterForcedRotationInputName { get { return m_CharacterForcedRotationInputName; } set { m_CharacterForcedRotationInputName = value; } }

        private ActiveInputEvent m_StartFreeMovementInputEvent;
        private ActiveInputEvent m_StopFreeMovementInputEvent;
        private ActiveInputEvent m_StartCameraForcedRotationInputEvent;
        private ActiveInputEvent m_StopCameraForcedRotationInputEvent;

        private bool m_FreeMovement;
        private bool m_ForcedFreeMovement;
        ...
        public override void ChangeViewType(bool activate, float pitch, float yaw, Quaternion baseCharacterRotation)
        {
            ...
            if (activate) {
                ...
                if (m_Handler != null) {
                    ...
                    m_StartCameraForcedRotationInputEvent = GenericObjectPool.Get<ActiveInputEvent>();
                    m_StartCameraForcedRotationInputEvent.Initialize(ActiveInputEvent.Type.ButtonDown, m_CharacterForcedRotationInputName, "OnRPGViewTypeStartForcedRotation");

                    m_StopCameraForcedRotationInputEvent = GenericObjectPool.Get<ActiveInputEvent>();
                    m_StopCameraForcedRotationInputEvent.Initialize(ActiveInputEvent.Type.ButtonUp, m_CharacterForcedRotationInputName, "OnRPGViewTypeStopForcedRotation");

                    m_Handler.RegisterInputEvent(m_StartFreeMovementInputEvent);
                    m_Handler.RegisterInputEvent(m_StartCameraForcedRotationInputEvent);
                }
                ...
                EventHandler.RegisterEvent(m_GameObject, "OnRPGViewTypeStartForcedRotation", OnStartForcedRotation);
                EventHandler.RegisterEvent(m_GameObject, "OnRPGViewTypeStopForcedRotation", OnStopForcedRotation);
                ...
                if (m_Handler != null) {
                    ...
                    if (m_ForcedFreeMovement) {
                        m_Handler.UnregisterAbilityInputEvent(m_StopCameraForcedRotationInputEvent);
                    } else {
                        m_Handler.UnregisterAbilityInputEvent(m_StartCameraForcedRotationInputEvent);
                    }

                    GenericObjectPool.Return(m_StartFreeMovementInputEvent);
                    GenericObjectPool.Return(m_StopFreeMovementInputEvent);
                    GenericObjectPool.Return(m_StartCameraForcedRotationInputEvent);
                    GenericObjectPool.Return(m_StopCameraForcedRotationInputEvent);
                }
                ...
                EventHandler.UnregisterEvent(m_GameObject, "OnRPGViewTypeStartForcedRotation", OnStartForcedRotation);
                EventHandler.UnregisterEvent(m_GameObject, "OnRPGViewTypeStopForcedRotation", OnStopForcedRotation);
        ...
        public override Quaternion Rotate(float horizontalMovement, float verticalMovement, bool immediatePosition)
        {
            ...
            if (m_RotateWithCharacter) {
                m_BaseRotation = m_CharacterLocomotion.Rotation;
                horizontalMovement = 0;
            }

            if (m_CharacterRotate || m_ForcedFreeMovement) {
                m_Yaw += horizontalMovement * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale;
            } else {
                ...
                if (m_FreeMovement) {
                    m_YawOffset += horizontalMovement * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale;
                } else if (m_CharacterLocomotion.Moving) {
                    m_YawOffset = Mathf.SmoothDamp(m_YawOffset, 0, ref m_YawSnapVelocity, m_YawSnapDamping * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale * TimeUtility.FramerateDeltaTime);
                }
                var deltaRotation = MathUtility.InverseTransformQuaternion(m_BaseRotation, m_CharacterLocomotion.Rotation);
            ...
            if (!m_FreeMovement && !m_ForcedFreeMovement && !m_CharacterRotate) {
                verticalMovement = 0;
            }
        ...
        /// <summary>
        /// Force the character rotation towards the camera rotation, and enable free movement.
        /// </summary>
        private void OnStartForcedRotation()
        {
            // Free movement may not be allowed if the character doesn't attack automatically.
            if (!m_AllowFreeMovement) {
                return;
            ...
            // Force the character to look in the same direction as the camera.
            var rpgMovementType = m_CharacterLocomotion.ActiveMovementType as Opsive.UltimateCharacterController.ThirdPersonController.Character.MovementTypes.RPG;
            if (rpgMovementType != null) {
                rpgMovementType.TurnWithLookSource(true);
            }

            // The camera should update immediately.
            m_Yaw = m_YawOffset = 0;
            m_CameraController.PositionImmediately(false);

            m_ForcedFreeMovement = true;

            // The ViewType doesn't need to listen for the start input anymore and should instead listen for the stop.
            if (m_Handler != null && m_AllowFreeMovement) {
                m_Handler.UnregisterAbilityInputEvent(m_StartCameraForcedRotationInputEvent);
                m_Handler.RegisterInputEvent(m_StopCameraForcedRotationInputEvent);
            }
        }
        ...
        /// The ViewType should no longer move freely around the yaw axis.
        /// </summary>
        private void OnStopForcedRotation()
        {
            var rpgMovementType = m_CharacterLocomotion.ActiveMovementType as Opsive.UltimateCharacterController.ThirdPersonController.Character.MovementTypes.RPG;
            if (rpgMovementType != null) {
                rpgMovementType.TurnWithLookSource(false);
            }

            m_ForcedFreeMovement = false;
