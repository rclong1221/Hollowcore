...
namespace Opsive.UltimateCharacterController.Items.Actions.Modules.Magic
{
        ...
        IModuleCanStartUseItem, IModuleCanUseItem, IModuleIsItemUsePending, IModuleCanStopItemUse, IModuleTryStopItemUse, IModuleStartItemUse, IModuleStopItemUse
    {
        ...
        protected override void Initialize(CharacterItemAction itemAction)
        {
            ...
            if (NetworkInfo != null && !NetworkInfo.IsLocalPlayer() && !NetworkInfo.IsSpectator()) {
                m_SurfaceIndicator = null;
            }
#endif
            MagicAction.OnLateUpdateEvent += LateUpdate;
        }
        ...
        public void OnAnimatorEndCast()
        {
            ...
            MagicAction.OnLateUpdateEvent -= CastLateUpdate;
        }
        ...
        {
            m_StartUseTime = TimeUtility.Time;
            m_StartCastTime = 0;
        ...
        public override void Cast(MagicUseDataStream magicUseDataStream)
        {
            ...
            m_StartCastTime =  TimeUtility.Time;
        ...
        protected virtual void StartCastEffects()
        {
            ...
            if (MagicAction.NetworkInfo != null && MagicAction.NetworkInfo.HasAuthority()) {
                MagicAction.NetworkCharacter.InvokeMagicCastEffectsModules(MagicAction, MagicAction.CastEffectsModuleGroup, invokedBitmask, 
                    Networking.Inventory.INetworkCharacterInventory.CastEffectState.Start, MagicAction.MagicUseDataStream);
            }
#endif
        }
        ...
        {
            CastUpdateInternal(false);
        }

        /// <summary>
        /// Tick the update function for casting during late update.
        /// </summary>
        private void CastLateUpdate()
        {
            CastUpdateInternal(true);
        }

        /// <summary>
        /// Updates the cast.
        /// </summary>
        /// <param name="lateUpdate">Is the cast being updated during late update?</param>
        private void CastUpdateInternal(bool lateUpdate)
        {
            ...
            if (!allActionsCasted || m_ContinuousCast) {
                ...
                for (int i = 0; i < useCount; ++i) {
                    ...
                    for (int j = 0; j < castEffectModules.Count; j++) {
                        if (lateUpdate) {
                            castEffectModules[j].OnCastLateUpdate(MagicAction.MagicUseDataStream);
                        } else {
                    ...
                    if (MagicAction.NetworkInfo != null && MagicAction.NetworkInfo.HasAuthority()) {
                        MagicAction.NetworkCharacter.InvokeMagicCastEffectsModules(MagicAction, MagicAction.CastEffectsModuleGroup, invokedBitmask,
                        Networking.Inventory.INetworkCharacterInventory.CastEffectState.Update, MagicAction.MagicUseDataStream);
                    }
#endif
                }
            }
            ...
            allActionsCasted = !m_ContinuousCast;
            if (allActionsCasted) {
                for (int i = 0; i < castEffectModules.Count; ++i) {
                    if (!castEffectModules[i].IsCastComplete(MagicAction.MagicUseDataStream)) {
                        allActionsCasted = false;
                    }
                }
        ...
        private bool DetermineCastValues(int index, ref Vector3 direction, ref Vector3 position, ref Vector3 normal)
        {
            ...
            if (m_Direction == CastDirection.Forward || m_Direction == CastDirection.Indicate) {
                if (m_Direction == CastDirection.Forward) {
                    direction = m_UseLookSource ? LookSource.LookDirection(CastOriginLocation.position, true, m_CharacterLayerManager.SolidObjectLayers, true, true) : CharacterTransform.forward;
        ...
        public virtual bool CanStopItemUse()
        {
            return m_UseType == CastUseType.Single || 
                   (m_StopRequested && (m_MinContinuousUseDuration == -1 || (m_MinContinuousUseDuration > 0 && m_StartUseTime + m_MinContinuousUseDuration < TimeUtility.Time)));
        }
