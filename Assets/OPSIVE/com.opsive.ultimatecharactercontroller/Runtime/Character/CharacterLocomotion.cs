...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class CharacterLocomotion : StateBehavior, IForceObject
    {
        ...
        [Tooltip("The amount of buffer to add to the initial vertical raycast. Increase this amount if your character can land on fast moving platforms.")]
        [SerializeField] protected float m_VerticalCastBuffer;
        ...
        [Tooltip("The multiplier to apply to the movement when leaving a moving platform.")]
        [SerializeField] protected float m_MovingPlatformDisconnectMovementMultiplier = 0.8f;
        ...
        public float MovingPlatformDisconnectMovementMultiplier { get => m_MovingPlatformDisconnectMovementMultiplier; set => m_MovingPlatformDisconnectMovementMultiplier = value; }
        ...
        protected bool m_AllowHorizontalCollisionDetection = true;
        protected bool m_ForceHorizontalCollisionDetection;
        protected bool m_AllowVerticalCollisionDetection = true;
        protected bool m_ForceVerticalCollisionDetection;
        protected bool m_AllowGroundCollisionDetection = true;
        protected bool m_ForceGroundCollisionDetection;
        protected bool m_AlignToUpDirection;
        ...
        protected Quaternion m_RootMotionDeltaRotation = Quaternion.identity;
        protected Quaternion m_PrevMotorRotation;

        protected Vector3 m_ExternalForce;
        protected Vector3[] m_SoftForceFrames;
        ...
        protected bool m_ApplyMovingPlatformDisconnectMovement;
        ...
        protected Quaternion m_Torque;
        private Action<RaycastHit> m_OnCollision;

        protected int m_SimulationIndex = -1;
        protected Transform m_RigidbodyParent;
        protected bool m_Interpolate = true;
        ...
        public Vector3 Center => this.InverseTransformPoint(m_Position + (m_Up * m_Height / 2));
        ...
        public Vector3 LocalVelocity => MathUtility.InverseTransformDirection(m_Velocity, m_Rotation * m_DesiredRotation);
        public Quaternion Torque => m_Torque;
        public Quaternion LocalTorque => MathUtility.InverseTransformQuaternion(m_Torque, m_Rotation * m_DesiredRotation);
        ...
        [Shared.Utility.NonSerialized] public Vector3 Position { get { return m_Position; } }
        [Shared.Utility.NonSerialized] public Quaternion Rotation { get { return m_Rotation; } }
        [Shared.Utility.NonSerialized] public Vector3 TargetPosition { get { return m_Position + m_DesiredMovement; } }
        [Shared.Utility.NonSerialized] public Quaternion TargetRotation { get { return m_Rotation * m_DesiredRotation; } }

        [Shared.Utility.NonSerialized] public Transform RigidbodyParent { get { return m_RigidbodyParent; } set { m_RigidbodyParent = value; } }
        ...
        public virtual void PreMove()
        {
            ...
            if (m_MovingPlatform != null) {
                ...
                if (m_AlignToUpDirection) {
                    ...
                    var localRotation = MathUtility.InverseTransformDirection(m_MovingPlatform.eulerAngles - m_PrevMovingPlatformRotation.eulerAngles, m_Rotation);
                    localRotation.x = localRotation.z = 0;
                    m_MovingPlatformRotation = Quaternion.Euler(MathUtility.TransformDirection(localRotation, m_Rotation));
                }

                // Update the position changes.
                m_PreMovingPlatformMovement = m_MovingPlatformMovement;
                m_MovingPlatformMovement = m_MovingPlatform.TransformPoint(m_MovingPlatformRelativePosition) - m_Position;
            }
        }
        ...
        {
            if (m_TimeScale == 0 || TimeUtility.DeltaTime == 0) {
                return;
            }
        ...
        private void UpdateMovingPlatformMovement()
        {
            ...
            if (!m_StickToMovingPlatform) {
                var prevMovingPlatformVelocity = m_PreMovingPlatformMovement / (m_TimeScale * TimeUtility.DeltaTime);
                var movingPlatformVelocity = m_MovingPlatformMovement / (m_TimeScale * TimeUtility.DeltaTime);
        ...
        protected virtual void UpdateRotation()
        {
            ...
            if (UsingRootMotionRotation) {
                ...
                targetRotation = Quaternion.Slerp(rotation, rotation * Quaternion.Euler(m_DeltaRotation), m_MotorRotationSpeed * m_TimeScale * TimeUtility.TimeScale);
            }
            ...
            if (m_AlignToUpDirection) {
                ...
                if (proj.sqrMagnitude > 0.0001f) {
                    if (m_MovingPlatform == null) {
                        targetRotation = Quaternion.Slerp(targetRotation, Quaternion.LookRotation(proj, m_Up), m_UpAlignmentRotationSpeed * m_TimeScale * TimeUtility.TimeScale);
                    } else {
        ...
        public Quaternion CheckRotation(Quaternion rotationDelta, bool forceCheck)
        {
            ...
            if (m_CheckRotationCollision && rotationDelta.eulerAngles.sqrMagnitude > 0) {
                ...
                for (int i = 0; i < m_ColliderCount; ++i) {
                    ...
                    for (int j = 0; j < m_MaxRotationCollisionChecks; ++j) {
                        ...
                        var targetPosition = MathUtility.TransformPoint(m_Position, m_Rotation, MathUtility.PositionFromMatrix(matrix));
                        var targetRotation = MathUtility.TransformQuaternion(m_Rotation, MathUtility.QuaternionFromMatrix(matrix));
                        ...
                        if (hitCount > 0) {
                            ...
                            for (int k = 0; k < hitCount; ++k) {
                                if (Physics.ComputePenetration(m_Colliders[i], targetPosition, targetRotation,
                                    ...
                                    var slope = Vector3.Angle((m_Rotation * rotationDelta) * Vector3.up, direction);
        ...
        public int OverlapColliders(Collider overlapCollider, Vector3 targetPosition, Quaternion targetRotation, float sizeBuffer = 0)
        {
            ...
            if (overlapCollider is CapsuleCollider) {
                ...
                hitCount = Physics.OverlapCapsuleNonAlloc(capsuleEndCap1 + m_Up * Mathf.Abs(sizeBuffer), capsuleEndCap2 - m_Up * Mathf.Abs(sizeBuffer), capsuleCollider.radius * MathUtility.ColliderScaleMultiplier(capsuleCollider),
        ...
        protected virtual void ApplyRotation()
        {
            ...
            if (m_MovingPlatform != null) {
                var movingPlatformRotation = m_MovingPlatform.rotation;
                m_MovingPlatformRotationOffset = MathUtility.InverseTransformQuaternion(m_Rotation, movingPlatformRotation);
                if (m_AlignToUpDirection) {
            ...
            m_DesiredRotation = m_RootMotionDeltaRotation = Quaternion.identity;
        }
        ...
        protected virtual void UpdatePosition()
        {
            ...
            if (UsingRootMotionPosition) {
                ...
                m_MotorThrottle = this.TransformDirection(localDeltaPosition) / TimeUtility.DeltaTime;
            } else {
                // Dampen motor forces.
                m_MotorThrottle /= (1 + m_MotorDamping * m_TimeScale * TimeUtility.TimeScale);
                ...
                var acceleration = m_SlopeFactor * backwardsMultiplier * m_MotorAcceleration * m_TimeScale * TimeUtility.TimeScale;
        ...
        {
            m_DesiredMovement += (m_MotorThrottle + m_ExternalForce) * TimeUtility.DeltaTime;

            if (!m_Grounded && UsingGravity) {
                m_GravityAccumulation += m_GravityAmount * m_TimeScale * TimeUtility.TimeScale * TimeUtility.DeltaTime;
                m_DesiredMovement += m_GravityDirection * (m_GravityAccumulation * m_TimeScale * TimeUtility.TimeScale);
            }
        }
        ...
        private void UpdateExternalForces()
        {
            ...
            m_ExternalForce /= (1 + m_ExternalForceDamping * m_TimeScale * TimeUtility.TimeScale);
        }
        ...
        private void DetectCollisions()
        {
            ...
            do {
                ...
                var hitCount = CombinedCast(lastPosition, normalizedTargetMovement, Mathf.Max(movementMagnitude, m_SkinWidth + c_ColliderSpacing));
                if (hitCount > 0) {
                    for (int i = 0; i < hitCount; ++i) {
                        ...
                        if (closestRaycastHit.distance == 0) {
                            if (ResolvePenetrations(activeCollider, targetPosition - m_Position, out var offset)) {
                                ...
                                if (!UsingGroundCollisionDetection || !UsingVerticalCollisionDetection || Mathf.Abs(Vector3.Dot(m_Up, this.InverseTransformDirection(closestRaycastHit.normal))) < 0.9995f) {
                                    ...
                                    settled = false;
                                    m_OnCollision?.Invoke(closestRaycastHit);
                                }
                            } else {
                        ...
                        if (Vector3.Dot(closestRaycastHit.normal, m_Up) > 0) {
                            ...
                            if (closestRaycastHit.collider.Raycast(new Ray(MathUtility.TransformPoint(closestHitPoint, m_Rotation, Vector3.forward * c_ColliderSpacing) + m_Up * m_MaxStepHeight,
                                ...
                                !Physics.Raycast(m_Position + m_Up * (m_MaxStepHeight + m_SkinWidth), normalizedTargetMovement, Mathf.Min(movementMagnitude, m_Radius), m_CharacterLayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore)) {
                                continue;
                            }
                        }
                        ...
                        var groundPoint = MathUtility.InverseTransformPoint(m_Position, m_Rotation * m_DesiredRotation, closestRaycastHit.point - m_MovingPlatformMovement);
                        if (groundPoint.y > 0 && (groundPoint.y <= m_MaxStepHeight || groundPoint.y < movementMagnitude)) {
                            if (slopeAngle >= 89.999f) {
                                ...
                                if (!SingleCast(activeCollider, (targetPosition - m_Position).normalized, m_MovingPlatformMovement + m_Up * (m_MaxStepHeight - c_ColliderSpacing),
                                ...
                                if (!Physics.Raycast(m_Position + m_Up * (m_MaxStepHeight + m_SkinWidth), normalizedTargetMovement, Mathf.Min(movementMagnitude, m_Radius), m_CharacterLayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore)) {
                                    m_SlopedGround = true;
                                    continue;
                                }
                            }
                        ...
                        var hitDistance = Mathf.Max(closestRaycastHit.distance - m_SkinWidth - c_ColliderSpacing, 0);
                        var minDistance = Mathf.Max(Mathf.Min(movementMagnitude, hitDistance), 0);
                        // Stop moving if a ceiling is near the character.
                        if ((Vector3.Dot(closestRaycastHit.normal, -m_Up) > 0.5)) {
                            minDistance = 0;
                            ...
                            if (m_WallBounceModifier > 0) {
                                ...
                                if (bouncinessValue > 0.0f) {
                                    var magnitude = m_DesiredMovement.magnitude * bouncinessValue * m_WallBounceModifier / TimeUtility.DeltaTime;
                            ...
                            if (hitDistance < movementMagnitude && (movementDot = Vector3.Dot(normalizedTargetMovement, projectedNormal)) > -0.99f && Vector3.Dot(m_DesiredMovement.normalized, -m_Up) < 0.99f) {
                                var cross = Vector3.Cross(projectedNormal, normalizedTargetMovement);
                                ...
                                if (forward.sqrMagnitude > 0) {
                                    ...
                                    targetPosition = lastPosition + (movementMagnitude - hitDistance) * m_WallGlideCurve.Evaluate(1 - movementDot) * dynamicFrictionValue * Vector3.ProjectOnPlane(forward, m_Up).normalized;
        ...
        protected void DetectGroundCollision(bool sendGroundedEvents = true)
        {
            ...
            var castOffset = Mathf.Max(m_Radius, m_MaxStepHeight) + (m_SlopedGround ? localDesiredMovement.magnitude : 0) + Mathf.Abs(localMovingPlatformMovement.y) + m_VerticalCastBuffer;
            ...
            var slopeOffset = localDesiredMovement.sqrMagnitude == 0 ? m_Radius : 0; // If the character is on a steep slope the standard cast distance may not be long enough.
            var hitCount = CombinedCast(targetPosition, m_GravityDirection,
                                        (stickyGround ? m_StickToGroundDistance : 0) +
                                        Mathf.Abs(localDesiredMovement.y) + m_SkinWidth + castOffset + slopeOffset);
            if (hitCount > 0) {
                for (int i = 0; i < hitCount; ++i) {
                    var closestRaycastHit = QuickSelect.SmallestK(m_CombinedCastResults, hitCount, i, m_CastHitComparer);
                    var activeCollider = m_ColliderCount > 1 ? m_Colliders[m_ColliderIndexMap[closestRaycastHit]] : m_Colliders[m_ColliderIndex];
                    if (closestRaycastHit.distance == 0) {
                        if (ResolvePenetrations(activeCollider, targetPosition - m_Position, out var offset) && closestRaycastHit.collider.Raycast(new Ray(targetPosition + m_Up * (castOffset + slopeOffset), m_GravityDirection), out var hit, Mathf.Infinity) &&
                    ...
                    grounded = !m_ForceUngrounded && (stickyGround || localDesiredMovement.y <= Mathf.Abs(distance) || (closestRaycastHit.transform.gameObject.layer == LayerManager.MovingPlatform && distance < m_SkinWidth)) &&
                                                    ((Mathf.Abs(localDesiredMovement.y) + (stickyGround ? (m_StickToGroundDistance + c_ColliderSpacing) : 0) > distance) || distance < (m_SkinWidth + slopeOffset + c_ColliderSpacing)) &&
                                                    (distance + Mathf.Min(localDesiredMovement.y, 0) <= (m_SkinWidth + slopeOffset + c_ColliderSpacing) || stickyGround);
                    if (grounded) {
                        ...
                        if (UsingVerticalCollisionDetection) {
                            ...
                            if (bouncinessValue > 0) {
                                var magnitude = desiredMagnitude * bouncinessValue * m_GroundBounceModifier / TimeUtility.DeltaTime;
            ...
            if (UsingVerticalCollisionDetection && UsingHorizontalCollisionDetection && localDesiredMovement.y > 0) {
                hitCount = CombinedCast(m_Position, m_Transform.up, localDesiredMovement.y + c_ColliderSpacing, -c_ColliderSpacing * 2);
                if (hitCount > 0) {
                    for (int i = 0; i < hitCount; ++i) {
                        var closestRaycastHit = QuickSelect.SmallestK(m_CombinedCastResults, hitCount, i, m_CastHitComparer);
                        if (closestRaycastHit.distance > 0 && closestRaycastHit.distance < m_SkinWidth + c_ColliderSpacing) {
                            // The vertical external force should be cancelled out.
                            localDesiredMovement.y = 0;
                            m_DesiredMovement = MathUtility.TransformDirection(localDesiredMovement, m_Rotation) + m_MovingPlatformMovement;
                            break;
        ...
        /// <param name="radiusOffset">The amount to modify the radius by.</param>
        /// <param name="drawDebugLine">Should the debug lines be drawn?</param>
        /// <returns>The number of objects hit from the cast.</returns>
        private int CombinedCast(Vector3 position, Vector3 direction, float distance, float radiusOffset = -c_ColliderSpacing
#if UNITY_EDITOR
            ...
            )
        {
            ...
            var rigidbodyRotation = m_Rotation * m_MovingPlatformRotation;
            var positionOffset = MathUtility.InverseTransformPoint(m_Position, rigidbodyRotation, position);
            var hitCount = 0;
            for (int i = 0; i < m_ColliderCount; ++i) {
                // The collider may not be enabled.
                if (!m_Colliders[i].gameObject.activeInHierarchy) {
                ...
                if (m_Colliders[i] is CapsuleCollider) {
                    ...
                    var radius = capsuleCollider.radius * MathUtility.ColliderScaleMultiplier(capsuleCollider) + radiusOffset;
                    ...
                    var radius = sphereCollider.radius * MathUtility.ColliderScaleMultiplier(sphereCollider) + radiusOffset;
                    ...
                    var boxCollider = m_Colliders[i] as BoxCollider;
                    var extents = (MathUtility.ColliderScaleMultiplier(boxCollider) + radiusOffset) * boxCollider.size / 2;
        ...
        {
            m_Velocity = (m_DesiredMovement - m_MovingPlatformMovement) / TimeUtility.DeltaTime;
        ...
        {
            return m_MovingPlatformMovement * m_MovingPlatformDisconnectMovementMultiplier;
        }
        ...
        /// </summary>
        /// <param name="force">The force to add.</param>
        protected void AddExternalForce(Vector3 force)
        {
        ...
        /// <param name="force">The force to add.</param>
        /// <param name="frames">The number of frames to add the force to.</param>
        protected void AddSoftForce(Vector3 force, float frames)
        {
        ...
        public void AddRelativeForce(Vector3 force, int frames = 1, bool scaleByMass = true)
        {
            ...
            AddForce(this.InverseTransformDirection(force), frames, scaleByMass);
        }
        ...
        {
            m_Transform.position = m_Rigidbody.position = m_Position = position;
        ...
        {
            m_Position = m_Rigidbody.position;
            m_Rotation = m_PrevMotorRotation = m_Rigidbody.rotation;
            m_Up = m_Rotation * Vector3.up;
            m_GravityDirection = -m_Up;
            m_DesiredRotation = Quaternion.identity;
            m_MotorThrottle = m_DesiredMovement = m_ExternalForce = Vector3.zero;
