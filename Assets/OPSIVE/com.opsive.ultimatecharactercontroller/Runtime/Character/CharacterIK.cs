...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class CharacterIK : CharacterIKBase
    {
...
#if ULTIMATE_CHARACTER_CONTROLLER_MULTIPLAYER
            m_NetworkInfo = m_CharacterGameObject.GetCachedComponent<INetworkInfo>();
#endif
        ...
        public override Vector3 GetDefaultLookAtPosition()
        {
            ...
            var lookDirection = m_LookSource.LookDirection(headPosition, false, m_LayerMask, true, false);
        ...
        public override void SetAbilityIKTarget(Transform target, IKGoal ikGoal, float duration)
        {
            ...
            m_StartInterpolation[(int)ikGoal] = TimeUtility.Time;
        ...
        private void PositionLowerBody()
        {
            ...
            m_HipsOffset = m_ImmediatePosition ? hipsOffset : Mathf.Lerp(m_HipsOffset, hipsOffset, m_HipsPositionAdjustmentSpeed * TimeUtility.DeltaTime);
            m_HipsPosition = m_Transform.InverseTransformPoint(m_Hips.position);
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                if (adjustmentSpeed == 0) {
                    ...
                    m_FootIKWeight[i] = Mathf.Clamp01(m_ImmediatePosition ? targetWeight : Mathf.MoveTowards(m_FootIKWeight[i], targetWeight, adjustmentSpeed * TimeUtility.DeltaTime));
                }
        ...
        private void RotateHands()
        {
            ...
            for (int i = 0; i < 2; ++i) {
...
#if ULTIMATE_CHARACTER_CONTROLLER_MULTIPLAYER
                var firstPersonPerspective = (m_NetworkInfo == null || m_NetworkInfo.IsLocalPlayer() || m_NetworkInfo.IsSpectator()) ? m_CharacterLocomotion.FirstPersonPerspective : false;
#else
                var firstPersonPerspective = m_CharacterLocomotion.FirstPersonPerspective;
#endif
                ...
                // is being used.
                var targetWeight = 0f;
                if (m_HandWeight > 0 && (ikTarget != null || m_Aiming || m_ItemInUse || firstPersonPerspective || m_OnUpdateIKRotation != null)) {
                    targetWeight = m_HandWeight;
                }

                m_HandRotationIKWeight[i] = Mathf.Clamp01(m_ImmediatePosition ? targetWeight :
                                                Mathf.MoveTowards(m_HandRotationIKWeight[i], targetWeight, m_HandAdjustmentSpeed * TimeUtility.DeltaTime));
        ...
        private void RotateUpperArms()
        {
            ...
            m_DominantUpperArmWeight = Mathf.Clamp01(m_ImmediatePosition ? targetWeight : Mathf.MoveTowards(m_DominantUpperArmWeight, targetWeight, m_UpperArmAdjustmentSpeed * TimeUtility.DeltaTime));
        ...
        private void UpdateTargetInterpolations()
        {
            ...
            for (int i = 0; i < (int)IKGoal.Last; ++i) {
                if (m_StartInterpolation[i] != -1) {
                    ...
                    var time = m_InterpolationDuration[i] > 0 ? Mathf.Clamp01((TimeUtility.Time - m_StartInterpolation[i]) / m_InterpolationDuration[i]) : 1;
        ...
        private void PositionHands()
        {
            for (int i = 0; i < 2; ++i) {
                ...
                // is being used.
                if (ikTarget != null || m_Aiming || m_ItemInUse || firstPersonPerspective || m_UpperArmWeight > 0 || m_OnUpdateIKPosition != null) {
                    targetWeight = m_HandWeight;
                }

                m_HandPositionIKWeight[i] = Mathf.Clamp01(m_ImmediatePosition ? targetWeight :
                                                Mathf.MoveTowards(m_HandPositionIKWeight[i], targetWeight, m_HandAdjustmentSpeed * TimeUtility.DeltaTime));
