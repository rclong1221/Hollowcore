...
#pragma warning restore 0649
        ...
        protected virtual void DetectBodyStep()
        {
            for (int i = 0; i < m_Feet.Length; ++i) {
                ...
                if (verticalOffset < m_LastVerticalOffset[i] && m_CharacterLocomotion.LocalVelocity.sqrMagnitude > m_CharacterLocomotion.ColliderSpacing) {
                    ...
                    if ((m_UpCount[i] > (m_MoveDirectionFrameCount  / (m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale)) || m_DownCount[i] > (m_MoveDirectionFrameCount / (m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale))) && 
                            m_Feet[i].Object != m_LastFootDown && FootStep(m_Feet[i].Object, m_Feet[i].FlippedFootprint)) {
                        m_LastFootDown = m_Feet[i].Object;
                        m_LastFootstepTime = TimeUtility.Time;
        ...
        protected virtual void UpdateFixedInterval()
        {
            ...
            if (m_LastFootstepTime + m_FixedInterval * m_CharacterLocomotion.TimeScale > TimeUtility.Time) {
                return;
            }
        ...
        protected virtual void DetectCameraBob()
        {
            ...
            if (m_LastVerticalOffset[0] >= m_VerticalOffset[0] && verticalOffset > m_VerticalOffset[0] && m_LastFootstepTime + (m_MinBobInterval * m_CharacterLocomotion.TimeScale) < TimeUtility.Time) {
                GroupFootStep();
                m_LastFootstepTime = TimeUtility.Time;
            }
        ...
        public bool TriggerFootStep(Transform foot, bool flipFootprint)
        {
            ...
            if ((!moving && m_RequireMovement) || m_LastFootstepTime + m_MinTriggerInterval > TimeUtility.Time || (m_RequireAlternatingFeet && foot == m_LastFootDown)) {
                return false;
            }
        ...
        public virtual bool FootStep(Transform foot, bool flipFootprint)
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(foot.position + m_Transform.up * 0.2f, -m_Transform.up, out hit, 0.21f + m_FootOffset + Mathf.Abs(m_CharacterLocomotion.LocalVelocity.y / TimeUtility.DeltaTime), m_CharacterLayerManager.IgnoreInvisibleCharacterWaterLayers, QueryTriggerInteraction.Ignore)) {
