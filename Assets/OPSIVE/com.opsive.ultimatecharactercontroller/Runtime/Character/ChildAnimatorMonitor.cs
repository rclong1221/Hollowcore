...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class ChildAnimatorMonitor : MonoBehaviour
    {
        ...
        private AnimationMonitorBase m_CharacterAnimationMonitor;
        ...
        private void Awake()
        {
            ...
            var animationMonitors = m_Character.GetComponentsInChildren<AnimationMonitorBase>(false);
            if (animationMonitors == null || animationMonitors.Length == 0) {
                Debug.LogError("Error: Unable to find an active Animator.", gameObject);
                return;
            }

            m_CharacterAnimationMonitor = animationMonitors[0];

            if (m_CharacterAnimationMonitor.HasItemParameters) {
                var slotCount = m_CharacterAnimationMonitor.ParameterSlotCount;
...
#if ULTIMATE_CHARACTER_CONTROLLER_VERSION_2_VR
#if FIRST_PERSON_CONTROLLER
            var handHandler = m_CharacterAnimationMonitor.GetComponent<IVRHandHandler>();
            ...
            enabled = m_CharacterAnimationMonitor != null;
            if (enabled) {
        ...
        public void CopyParameters(AnimationMonitorBase targetAnimatorMonitor)
        {
            if (!m_Initialized) {
                Awake();
            }
        ...
        public void SnapAnimator(bool executeEvent)
        {
            ...
            m_HorizontalMovement = m_CharacterAnimationMonitor.HorizontalMovement;
            m_ForwardMovement = m_CharacterAnimationMonitor.ForwardMovement;
            m_Pitch = m_CharacterAnimationMonitor.Pitch;
            m_Yaw = m_CharacterAnimationMonitor.Yaw;
            m_Speed = m_CharacterAnimationMonitor.Speed;
            m_Height = m_CharacterAnimationMonitor.Height;
            m_Moving = m_CharacterAnimationMonitor.Moving;
            m_Aiming = m_CharacterAnimationMonitor.Aiming;
            m_MovementSetID = m_CharacterAnimationMonitor.MovementSetID;
            m_AbilityIndex = m_CharacterAnimationMonitor.AbilityIndex;
            m_AbilityIntData = m_CharacterAnimationMonitor.AbilityIntData;
            m_AbilityFloatData = m_CharacterAnimationMonitor.AbilityFloatData;
            ...
            m_Animator.SetBool(s_AbilityChangeHash, true);
            ...
            if (m_CharacterAnimationMonitor.HasItemParameters) {
                for (int i = 0; i < m_CharacterAnimationMonitor.ParameterSlotCount; ++i) {
                    if (!m_ItemParameterExists.Contains(i)) {
                        continue;
                    }

                    m_ItemSlotID[i] = m_CharacterAnimationMonitor.ItemSlotID[i];
                    m_ItemSlotStateIndex[i] = m_CharacterAnimationMonitor.ItemSlotStateIndex[i];
                    m_ItemSlotSubstateIndex[i] = m_CharacterAnimationMonitor.ItemSlotSubstateIndex[i];
...
#endif
#if UNITY_EDITOR
            if (m_CharacterAnimationMonitor.LogEvents) {
        ...
        {
            m_CharacterAnimationMonitor = activeModel.GetCachedComponent<AnimatorMonitor>();
        }
        ...
        {
            if (m_CharacterAnimationMonitor != null) {
