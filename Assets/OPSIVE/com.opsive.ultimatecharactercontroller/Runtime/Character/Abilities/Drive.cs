...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public class Drive : DetectObjectAbilityBase
    {
        ...
        public AnimationMonitorBase CharacterAnimationMonitor { get => m_AnimationMonitor; }
        ...
        protected override void AbilityStarted()
        {
            ...
            if (m_TeleportEnterExit) {
                ...
                var forward = Vector3.ProjectOnPlane(location.transform.forward, m_CharacterLocomotion.Up);
                m_CharacterLocomotion.SetPositionAndRotation(location.position, Quaternion.LookRotation(forward, m_CharacterLocomotion.Up), true, false);
            }
        ...
        private void OnEnteredVehicle()
        {
            ...
            m_CharacterLocomotion.RigidbodyParent = m_DriveSource.Transform;
            UpdateAbilityAnimatorParameters();

            // Use the interpolation settings set on the Rigidbody.
            m_StartInterpolation = m_CharacterLocomotion.Interpolate;
            var driveRigidbody = m_DriveSource.GameObject.GetCachedComponent<Rigidbody>();
            if (driveRigidbody != null) {
                m_CharacterLocomotion.Interpolate = driveRigidbody.interpolation == RigidbodyInterpolation.Interpolate;
            }
        ...
        public override void Update()
        {
            ...
            if (m_DriveState == DriveState.ExitComplete && !m_TeleportEnterExit) {
                StopAbility();
            } else if (m_DriveState == DriveState.Enter) {
                m_CharacterLocomotion.InputVector = Vector3.zero;
            }

            // The horizontal input value can be used to animate the steering wheel.
            SetAbilityFloatDataParameter(m_CharacterLocomotion.RawInputVector.x, TimeUtility.DeltaTime);
        }
        ...
        {
            var targetRotation = m_Transform.rotation * m_CharacterLocomotion.DesiredRotation;
            ...
            m_CharacterLocomotion.DesiredRotation = Quaternion.Inverse(m_Transform.rotation) * targetRotation;
        }
        ...
        public override void UpdatePosition()
        {
            ...
            m_CharacterLocomotion.DesiredMovement = m_DriveSource.DriverLocation.position - m_Transform.position;
        }
        ...
        public override void WillTryStopAbility()
        {
            ...
            m_CharacterLocomotion.RigidbodyParent = null;
