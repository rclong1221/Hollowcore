...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public abstract class Ability : StateObject
    {
        ...
        [HideInInspector] [SerializeField] [StateName] protected string m_State;
        [Tooltip("Should the ItemIdentifier name be appened to the name of the state name?")]
        [HideInInspector] [LabelOverride("Append Item")] [SerializeField] protected bool m_StateAppendItemIdentifierName;
        ...
        [HideInInspector] [LabelOverride("Start")] [SerializeField] protected AudioClipSet m_StartAudioClipSet = new AudioClipSet();
        [Tooltip("A set of AudioClips that can be played when the ability is started.")]
        [HideInInspector] [LabelOverride("Stop")] [SerializeField] protected AudioClipSet m_StopAudioClipSet = new AudioClipSet();
        ...
        [Tooltip("Should the character detect ground collisions while the ability is active?")]
        [HideInInspector] [SerializeField] protected AbilityBoolOverride m_DetectGroundCollisions = AbilityBoolOverride.NoOverride;
        ...
        public bool Enabled { get { return m_Enabled; } set { m_Enabled = value; if (!m_Enabled && IsActive && CanStopAbility(true)) { StopAbility(true, false); } SetEnabled(value); } }
        ...
        public AbilityStopType StopType { get => m_StopType; set => m_StopType = value; }
        ...
        public AbilityBoolOverride DetectGroundCollisions { get { return m_DetectGroundCollisions; } set { m_DetectGroundCollisions = value; } }
        ...
        public virtual Sprite AbilityMessageIcon { get { return m_AbilityMessageIcon; }
            set {
        ...
        protected AnimationMonitorBase m_AnimationMonitor;
        ...
        public virtual void Initialize(UltimateCharacterLocomotion characterLocomotion, int index)
        {
            ...
            if (modelManager != null) {
                m_AnimationMonitor = modelManager.ActiveModel.GetCachedComponent<AnimationMonitorBase>();
            } else {
                m_AnimationMonitor = m_GameObject.GetComponentInChildren<AnimationMonitorBase>();
            }
            ...
            if (m_AbilityStarter != null) {
                m_AbilityStarter.Initialize(m_GameObject, this);
            }
            if (m_AbilityStopper != null) {
                m_AbilityStopper.Initialize(m_GameObject, this);
            }
        ...
        public virtual bool CanInputStartAbility(IPlayerInput playerInput)
        {
            if (m_StartType == AbilityStartType.Custom && m_AbilityStarter != null) {
                return m_AbilityStarter.CanInputStartAbility(playerInput);
            }
        ...
        {
            if (m_StopType == AbilityStopType.Custom && m_AbilityStopper != null) {
        ...
        {
            return CanStartAbilityInternal();
        }

        /// <summary>
        /// Internal method which is called when the ablity is tried to be started. If false is returned then the ability will not be started.
        /// </summary>
        /// <returns>True if the ability can be started.</returns>
        protected bool CanStartAbilityInternal()
        {
            if (m_AttributeModifier != null && !m_AttributeModifier.IsValid()) { return false; }
            if (m_CharacterLocomotion != null && !m_CharacterLocomotion.Alive && !CanStayActivatedOnDeath) { return false; }
            return true;
        }
        ...
        protected void AbilityStarted(bool enableAttributeModifier)
        {
            ...
            if (m_InputIndex != -1) {
                ...
                if (m_StopType != AbilityStopType.Automatic && m_StopType != AbilityStopType.Manual && m_StopType != AbilityStopType.Custom) {
                    for (int i = 0; i < m_ActiveInput.Length; ++i) {
            ...
            if (m_DetectGroundCollisions == AbilityBoolOverride.True) {
                m_CharacterLocomotion.ForceGroundCollisionDetection = true;
            } else if (m_DetectGroundCollisions == AbilityBoolOverride.False) {
                m_CharacterLocomotion.AllowGroundCollisionDetection = false;
            }
            m_StartTime = TimeUtility.Time;
            m_ActiveCount++;
        ...
        private void OnEquipItem(CharacterItem characterItem, int slotID)
        {
            if (IsActive && !string.IsNullOrEmpty(m_State)) {
                var itemStateName = m_State + characterItem.ItemDefinition.name;
        ...
        public virtual void Update()
        {
            ...
            if (m_AnimatorMotion != null) {
                ...
                var deltaPosition = m_CharacterLocomotion.RootMotionDeltaPosition;
                m_AnimatorMotion.EvaluatePosition(TimeUtility.Time - m_StartTime, ref deltaPosition);
                m_CharacterLocomotion.RootMotionDeltaPosition += m_CharacterLocomotion.TransformDirection(deltaPosition);

                var deltaRotation = m_CharacterLocomotion.RootMotionDeltaRotation;
                m_AnimatorMotion.EvaluateRotation(TimeUtility.Time - m_StartTime, ref deltaRotation);
                m_CharacterLocomotion.RootMotionDeltaRotation *= deltaRotation;
            }
        }
        ...
        /// </summary>
        [System.Obsolete("Ability.UpdateAnimator has been deprecated. The Animator should be updated within Ability.Update instead.")]
        ...
        /// <summary>
        /// Verify the position values. Called immediately before the DesiredMovement is set.
        /// </summary>
        ...
        /// </summary>
        public virtual void UpdateDesiredMovement() { }
        ...
        public virtual bool CanStopAbility(bool force) { return true; }
        ...
        protected virtual void AbilityStopped(bool force)
        {
            ...
            if (m_DetectGroundCollisions == AbilityBoolOverride.True) {
                m_CharacterLocomotion.ForceGroundCollisionDetection = false;
            } else if (m_DetectGroundCollisions == AbilityBoolOverride.False) {
                m_CharacterLocomotion.AllowGroundCollisionDetection = true;
            }
        ...
        /// </summary>
        /// <param name="playerInput">A reference to the input component.</param>
        public void CheckInput(IPlayerInput playerInput)
        {
            if (m_ButtonUp == null) {
                return;
            }
        ...
        protected void AddForce(Vector3 force, int frames = 1, bool scaleByMass = true)
        {
            m_CharacterLocomotion.AddForce(force, frames, scaleByMass);
        }
        ...
        protected void AddRelativeForce(Vector3 force, int frames = 1, bool scaleByMass = true)
        {
            m_CharacterLocomotion.AddRelativeForce(force, frames, scaleByMass);
        }
        ...
        {
            // An AnimationMonitor is not required.
            if (m_AnimationMonitor == null) {
                return;
            }

            m_AnimationMonitor.SetAbilityIndexParameter(value);
        }
        ...
        protected void SetAbilityIntDataParameter(int value)
        {
            ...
            m_AnimationMonitor.SetAbilityIntDataParameter(value);
        }
        ...
        protected void SetAbilityFloatDataParameter(float value)
        {
            ...
            m_AnimationMonitor.SetAbilityFloatDataParameter(value, m_CharacterLocomotion.TimeScale);
        }
        ...
        protected void SetAbilityFloatDataParameter(float value, float dampingTime)
        {
            ...
            m_AnimationMonitor.SetAbilityFloatDataParameter(value, m_CharacterLocomotion.TimeScale, dampingTime);
        }
        ...
        protected void SetSpeedParameter(float value)
        {
            ...
            m_AnimationMonitor.SpeedParameterOverride = value != 0;
            if (value != 0) {
                m_AnimationMonitor.SetSpeedParameter(value, m_CharacterLocomotion.TimeScale);
            }
        }
        ...
        protected void SetHeightParameter(int value)
        {
            ...
            m_AnimationMonitor.SetHeightParameter(value);
        }

        /// <summary>
        /// Helper method for calling the required event for updating the ability parameters.
        /// </summary>
        /// <param name="immediateUpdate">Should the parameters be updated immediately?</param>
        protected void UpdateAbilityAnimatorParameters(bool immediateUpdate = false)
        {
            EventHandler.ExecuteEvent(m_GameObject, "OnCharacterUpdateAbilityParameters", immediateUpdate);
        }
        ...
        {
            m_AnimationMonitor = activeModel.GetCachedComponent<AnimationMonitorBase>();
