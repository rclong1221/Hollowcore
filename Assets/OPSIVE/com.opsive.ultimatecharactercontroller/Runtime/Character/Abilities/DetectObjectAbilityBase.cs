...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public abstract class DetectObjectAbilityBase : Ability
    {
        ...
        public int MaxTriggerObjectCount { get { return m_MaxTriggerObjectCount; } 
            set { 
                if (m_MaxTriggerObjectCount == value) { return; }
                m_MaxTriggerObjectCount = value; 
                if (m_DetectedTriggerObjects != null) {
                    System.Array.Resize(ref m_DetectedTriggerObjects, m_MaxTriggerObjectCount);
                }
            } }
        public bool MoveWithObject { get { return m_MoveWithObject; }
            set {
                ...
                if (m_MoveWithObject && m_CharacterLocomotion.MovingPlatform == null) {
                    m_CharacterLocomotion.SetMovingPlatform(m_DetectedObject.transform, true);
                } else if (!m_MoveWithObject && m_CharacterLocomotion.MovingPlatform == m_DetectedObject.transform) {
        ...
        public GameObject DetectedObject { get { return m_DetectedObject; }
            protected set
            {
                ...
                if (m_MoveWithObject && m_DetectedObject != null && m_CharacterLocomotion.MovingPlatform == m_DetectedObject) {
                    m_CharacterLocomotion.SetMovingPlatform(m_DetectedObject.transform, true);
                }
        ...
        public override bool CanStartAbility()
        {
            ...
            if ((m_ObjectDetection & ObjectDetectionMode.Charactercast) != 0) {
                if (m_CharacterLocomotion.SingleCast(castDirection, castTransform.TransformDirection(m_CastOffset), m_CastDistance, m_DetectLayers, ref m_RaycastResult)) {
        ...
        protected override void AbilityStarted()
        {
            ...
            if (m_MoveWithObject && m_DetectedObject != null && m_CharacterLocomotion.MovingPlatform == null) {
                m_CharacterLocomotion.SetMovingPlatform(m_DetectedObject.transform, true);
            }
        }
        ...
        protected void AbilityStopped(bool force, bool triggerExit)
        {
            ...
            if (m_MoveWithObject && m_DetectedObject != null && m_CharacterLocomotion.MovingPlatform == m_DetectedObject.transform) {
                m_CharacterLocomotion.SetMovingPlatform(null);
            }
