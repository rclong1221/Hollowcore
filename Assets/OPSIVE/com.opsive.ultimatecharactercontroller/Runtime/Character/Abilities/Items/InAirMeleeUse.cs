...
namespace Opsive.UltimateCharacterController.Character.Abilities.Items
{
    ...
    /// <summary>
    /// ItemAbility which will start using the MeleeAction while in the air.
    /// </summary>
    ...
    public class InAirMeleeUse : Use
    {
        ...
        private void OnGrounded(bool grounded)
        {
            ...
            if (grounded) {
                ...
                for (int i = 0; i < m_UsableItems.Length; ++i) {
                    var usableItem = m_UsableItems[i];
                    if (usableItem == null) { continue; }
                    if (usableItem.WaitingForUseEvent) {
                        usableItem.UseEvent.InvokeSlotEvent();
                    }
                }

                SurfaceManager.SpawnEffect(m_CharacterLocomotion.GroundedRaycastHit, m_GroundImpact, m_CharacterLocomotion.GravityDirection, m_CharacterLocomotion.TimeScale, m_GameObject);
