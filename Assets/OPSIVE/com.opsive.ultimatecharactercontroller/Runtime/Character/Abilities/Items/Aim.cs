...
namespace Opsive.UltimateCharacterController.Character.Abilities.Items
{
    ...
    public class Aim : ItemAbility
    {
        ...
        public override int GetItemSubstateIndex(int slotID)
        {
            ...
            if (item != null && item.DominantItem) {
                ...
                for (int i = 0; i < itemActions.Length; ++i) {
                    ...
                    if (usableItem != null) {
                        var substateIndex = usableItem.GetUseItemSubstateIndex();
                        if (substateIndex != -1) {
        ...
        public override bool CanInputStopAbility(IPlayerInput playerInput)
        {
        ...
        {
            if (force && !m_CharacterLocomotion.Alive) { return true; }
        ...
        {
#if UNITY_EDITOR
            if (IsActive && itemAbility.Index > Index && (itemAbility is Use || itemAbility is Reload)) {
        ...
        protected override void OnSwitchModels(GameObject activeModel)
        {
            ...
            // Temporarily reset the ActiveIndex if the input didn't start the aim ability. This will prevent the active state from unnecessarily being set.
            if (!m_InputStart) {
                ActiveIndex = -1;
            }
