...
namespace Opsive.UltimateCharacterController.Character.Abilities.Items
{
    ...
    public class EquipNext : EquipSwitcher
    {
        ...
        public override void Awake()
        {
            ...
            EventHandler.RegisterEvent<CharacterItem, bool>(m_GameObject, "OnNextItemSet", OnNextItemSet);
        }
        ...
        {
            if (itemSetIndex == -1 || 
                (m_ItemSetIndex != -1 && itemSetIndex == m_ItemSetManager.GetDefaultItemSetIndex(m_ItemSetGroupIndex) && !m_ItemSetManager.ItemSetGroups[m_ItemSetGroupIndex].ItemSetList[itemSetIndex].CanSwitchTo)) {
                return;
            }
        ...
        public override bool CanStartAbility()
        {
            ...
            m_ItemSetIndex = m_ItemSetManager.NextActiveItemSetIndex(m_ItemSetGroupIndex, m_PrevItemSetIndex, true);
        ...
        /// </summary>
        /// <param name="characterItem">The item that should be changed.</param>
        /// <param name="unequipOnFailure">Should the current ItemSet be unequipped if the next ItemSet cannot be activated?</param>
        private void OnNextItemSet(CharacterItem characterItem, bool unequipOnFailure)
        {
            var itemIdentifier = characterItem.ItemIdentifier;
            if (itemIdentifier == null || !m_ItemSetManager.IsCategoryMember(itemIdentifier.GetItemDefinition(), m_ItemSetGroupIndex)) {
                return;
            }

            // Don't equip the next item if the current ItemSet doesn't contain the ItemIdentifier.
            var activeItemIdentifier = m_ItemSetManager.GetEquipItemIdentifier(m_ItemSetGroupIndex, characterItem.SlotID);
            ...
            if (!StartAbility() && unequipOnFailure) {
                m_EquipUnequipItemAbility.StartEquipUnequip(m_ItemSetManager.GetDefaultItemSetIndex(m_ItemSetGroupIndex));
            }
        }
        ...
        public override void OnDestroy()
        {
            ...
            if (m_EquipUnequipItemAbility == null) {
                EventHandler.UnregisterEvent<CharacterItem, bool>(m_GameObject, "OnNextItemSet", OnNextItemSet);
            }
        }
    }
}