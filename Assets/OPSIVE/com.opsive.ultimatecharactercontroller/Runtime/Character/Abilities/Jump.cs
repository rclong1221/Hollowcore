...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public class Jump : Ability
    {
        ...
        [Tooltip("Should the gravity accumulation be reset when the Jump ability ends?")]
        [SerializeField] protected bool m_ResetGravityAccumulation = true;
        ...
        public bool ResetGravityAccumulation { get { return m_ResetGravityAccumulation; } set { m_ResetGravityAccumulation = value; } }
        ...
        protected float m_ActiveForce;
        protected float[] m_ActiveSoftForceFrames;
        private RaycastHit m_RaycastResult;
        protected bool m_ImmediateJump;
        protected bool m_Jumping;
        protected bool m_ApplyHoldForce;
        protected float m_HoldForce;
        protected int m_AirborneJumpCount;
        protected bool m_AirborneJumpApplied;
        protected float m_JumpTime = -1;
        protected float m_LandTime = -1;
        protected float m_InAirTime = -1;
        ...
        public override float AbilityFloatData { get { if (m_Jumping) { return m_CharacterLocomotion.LocalVelocity.y; } return -1; } }
        ...
        public override bool CanStartAbility()
        {
            ...
            if (m_CharacterLocomotion.Grounded) {
                ...
                if (m_LandTime + m_RecurrenceDelay >= TimeUtility.Time) {
                    return false;
                }
                ...
                if ((!m_Jumping && m_GroundedGracePeriod != -1 && m_InAirTime + m_GroundedGracePeriod <= TimeUtility.Time) || 
                    (m_AirborneJumpCount != -1 && m_AirborneJumpCount > m_MaxAirborneJumpCount) || (m_JumpTime != -1 && m_JumpTime + m_RecurrenceDelay > TimeUtility.Time)) {
                    return false;
                }
            }
        ...
        private void OnGrounded(bool grounded)
        {
            if (grounded) {
                ...
                m_LandTime = TimeUtility.Time + TimeUtility.DeltaTime;
                m_InAirTime = -1;
        ...
        private void ApplyJumpForce()
        {
            ...
            if (IsActive && !m_Jumping) {
                ...
                Scheduler.ScheduleFixed(TimeUtility.DeltaTime * 10, EnsureAirborne); 
            }
        }
        ...
        {
            if (IsActive && !m_CharacterLocomotion.Grounded) { // The character may not have left the ground before an airborne jump is requested.
                OnAirborneJump();
            } else {
                StartAbility();
            }
        ...
        private void OnAirborneJump()
        {
            ...
            m_AirborneJumpCount++;
            m_JumpTime = TimeUtility.Time;
            m_InAirTime = TimeUtility.Time; // Reset the InAirTime so the Jump ability can end.
        ...
        public override void UpdatePosition()
        {
            ...
            desiredMovement.y += m_ActiveForce * (m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale);
            ...
            m_ActiveForce -= (m_ForceDamping * 0.1f * (m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale));
        }
