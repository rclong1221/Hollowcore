...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public class ItemEquipVerifier : Ability
    {
                ...
                if (ability.AllowEquippedSlotsMask != -1) {
                    ...
                    for (int i = 0; i < m_Inventory.SlotCount; ++i) {
                        if ((characterItem = m_Inventory.GetActiveCharacterItem(i)) != null) {
                            ...
                            if (IsDefaultItemIdentifier(characterItem.ItemIdentifier)) {
                                continue;
                            }
                    ...
                    for (int i = 0; i < m_Inventory.SlotCount; ++i) {
                        if ((characterItem = m_Inventory.GetActiveCharacterItem(i)) != null) {
                            ...
                            for (int j = 0; j < ability.AllowItemDefinitions.Length; ++j) {
                                ...
                                if (characterItem.ItemIdentifier.GetItemDefinition() == ability.AllowItemDefinitions[j]) {
        ...
        public override bool ShouldBlockAbilityStart(Ability startingAbility)
        {
            ...
            if (m_StartingAbility == null || startingAbility is EquipUnequip || startingAbility.ImmediateStartItemVerifier) {
                return false;
            }
        ...
        {
            for (int i = 0; i < m_EquipUnequipAbilities.Length; ++i) {
                var itemSetCategoryIndex = m_EquipUnequipAbilities[i].ItemSetGroupIndex;
                if (m_Equip) {
                    // Don't equip if the character has since changed which item set is equipped.
                    if (m_ItemSetManager.GetActiveItemSetIndex(itemSetCategoryIndex) != m_TargetItemSetIndex[i]) {
                        continue;
                    }
                    m_EquipUnequipAbilities[i].StartEquipUnequip(m_StartingItemSetIndex[i], true, immediateEquipUnequip);
                } else {
                    var targetItemSetIndex = m_ItemSetManager.GetTargetItemSetIndex(itemSetCategoryIndex, m_AllowEquippedSlotsMask);
                    var activeItemSetIndex = m_ItemSetManager.GetActiveItemSetIndex(itemSetCategoryIndex);
