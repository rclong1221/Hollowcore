    ...
    public class DamageIndicatorMonitor : CharacterMonitor
    {
        ...
        private struct DamageIndicator
        {
            ...
            public void Initialize(Transform attacker, Vector3 position, GameObject gameObject)
            {
                ...
                m_DisplayTime = TimeUtility.Time;
        ...
        private void OnDamage(float amount, Vector3 position, Vector3 force, GameObject attacker, Collider hitCollider)
        {
            ...
            for (int i = 0; i < m_ActiveDamageIndicatorCount; ++i) {
                ...
                if (Mathf.Abs(angle - damageIndicator.Angle) < m_IndicatorAngleThreshold) {
                    damageIndicator.DisplayTime = TimeUtility.Time;
        ...
        private void Update()
        {
            for (int i = m_ActiveDamageIndicatorCount - 1; i > -1; --i) {
                ...
                var alpha = (m_IndicatorFadeTime - (TimeUtility.Time - (m_ActiveDamageIndicators[i].DisplayTime + m_IndicatorVisiblityTime))) / m_IndicatorFadeTime;
                if (alpha <= 0 || m_ActiveDamageIndicators[i].Attacker == null) {
