...
namespace Opsive.UltimateCharacterController.UI
{
    ...
    public class HealthFlashMonitor : CharacterMonitor
    {
        ...
        protected virtual void OnDamage(float amount, Vector3 position, Vector3 force, GameObject attacker, Collider hitCollider)
        {
            if (!m_DamageFlash.CanActivate || amount <= 0) {
                return;
            }
            ...
            m_FlashDisplayTime = TimeUtility.Time;
        ...
        private void Update()
        {
            ...
            var alpha = Mathf.Min((m_ActiveFlash.FadeDuration - (TimeUtility.Time - (m_FlashDisplayTime + m_ActiveFlash.VisiblityDuration))) / m_ActiveFlash.FadeDuration, 1) * m_ActiveFlash.Color.a;
