...
namespace Opsive.UltimateCharacterController.FirstPersonController.Camera.ViewTypes
{
    ...
    [Shared.StateSystem.AddState("Zoom", "f08c1d8b08898574baa7bd27c1b05e62")]
    [System.Serializable]
    public class Combat : FirstPerson
    {
        ...
        public override Quaternion Rotate(float horizontalMovement, float verticalMovement, bool immediatePosition)
        {
            ...
            var rotation = base.Rotate(horizontalMovement, verticalMovement, immediatePosition);
            rotation = (m_CharacterTransform.rotation * Quaternion.Inverse(m_CharacterLocomotion.Rotation)) * rotation;
            return rotation;
        }
        ...
        private void OnAbilityActive(Ability ability, bool active)
        {
            ...
            // Rotate with the character so the camera will follow the character's rotation when the character is getting into position for Move Towards.
