...
namespace Opsive.UltimateCharacterController.FirstPersonController.Camera.ViewTypes
{
    ...
    public abstract class FirstPerson : ViewType
	{
        ...
        [NonSerialized] public ObjectOverlayRenderType OverlayRenderType
        {
            get { return m_OverlayRenderType; }
            set
            {
                m_OverlayRenderType = value;
        ...
        public override bool UseSmoothOffset { get { return m_CharacterLocomotion.RigidbodyParent != null; } }
        ...
        /// </summary>
        protected void InitializePositionSpringValue()
        {
        ...
        public override void UpdateFieldOfView(bool immediateUpdate)
        {
            ...
            if (m_FirstPersonCamera != null) {
                if (m_SynchronizeFieldOfView) {
                    ...
                    var zoom = immediateUpdate ? 1 : ((TimeUtility.Time - m_FirstPersonFieldOfViewChangeTime) / (m_FirstPersonFieldOfViewDamping / m_CharacterLocomotion.TimeScale));
            ...
            if (m_CharacterLocomotion.AlignToUpDirection) {
                ...
                if (proj.sqrMagnitude > 0.0001f) {
                    if (m_CharacterLocomotion.MovingPlatform == null) {
                        m_BaseRotation = Quaternion.Slerp(m_BaseRotation, Quaternion.LookRotation(proj, m_CharacterLocomotion.Up), m_SecondaryRotationSpeed * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale);
                    } else {
        ...
        protected virtual void UpdateShakes()
        {
            ...
            m_Yaw += m_Shake.x;
            m_Pitch += m_Shake.y;
            m_RotationSpring.AddForce(m_Shake.z * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale * Vector3.forward);
        }
        ...
        private void UpdateBob()
        {
            ...
            currentBobOffsetValue.x = Mathf.Cos(m_BobPositionalRate.x * m_CharacterLocomotion.TimeScale * TimeUtility.Time * 10) * currentPositionalBobAmplitude.x;
            currentBobOffsetValue.y = Mathf.Cos(m_BobPositionalRate.y * m_CharacterLocomotion.TimeScale * TimeUtility.Time * 10) * currentPositionalBobAmplitude.y;
            currentBobOffsetValue.z = Mathf.Cos(m_BobPositionalRate.z * m_CharacterLocomotion.TimeScale * TimeUtility.Time * 10) * currentPositionalBobAmplitude.z;
            var currentBobRollValue = Mathf.Cos(m_BobRollRate * m_CharacterLocomotion.TimeScale * TimeUtility.Time * 10) * currentRollBobAmplitude;
        ...
        {
            var localVelocity = m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale * m_Transform.InverseTransformDirection(m_CharacterLocomotion.Velocity * 0.016f);
        ...
        public override void StateChange()
        {
            ...
            if (!string.IsNullOrEmpty(m_CameraController.ZoomState) && !m_AppendingZoomState && m_CameraController.ActiveViewType == this) {
            ...
            if (m_FirstPersonCamera != null && m_FirstPersonCamera.fieldOfView != m_FirstPersonFieldOfView) {
                m_FirstPersonFieldOfViewChangeTime = TimeUtility.Time;
                // The field of view should get a head start if the damping was previously 0. This will allow the field of view to move back to the original value even
                // when the state is no longer active.
