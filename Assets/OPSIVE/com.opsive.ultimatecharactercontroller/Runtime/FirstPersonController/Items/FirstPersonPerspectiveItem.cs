...
namespace Opsive.UltimateCharacterController.FirstPersonController.Items
{
    ...
    public class FirstPersonPerspectiveItem : PerspectiveItem
    {
        ...
        [SerializeField] protected IDObject<FirstPersonBaseObject>[] m_AdditionalControlObjects;
        ...
        [Tooltip("The base positional offset for the active character. This will be added to the PositionOffset value.")]
        [SerializeField] protected Vector3 m_BasePositionOffset;
        ...
        [Tooltip("An offset relative to the parent pivot TransforhasItemm. This is the desired position when the item is moving off screen (in the case of unequipping).")]
        ...
        [SerializeField] protected Vector3 m_RotationLookSway = new Vector3(20, 20, 0);
        ...
        [Shared.Utility.NonSerialized] public GameObject VisibleItem { get { return m_VisibleItem; } set { m_VisibleItem = value; } }
#if ULTIMATE_CHARACTER_CONTROLLER_VERSION_2_VR
        [NonSerialized] public bool VRHandParent { get { return m_VRHandParent; } set { m_VRHandParent = value; } }
#endif
        public Transform[] AdditionalControlObjectsTransforms { get { return m_AdditionalControlObjectsTransforms; } }
        public IDObject<FirstPersonBaseObject>[] AdditionalControlObjectsIDObjects { get { return m_AdditionalControlObjects; } }
        public Spring PositionSpring { get { return m_PositionSpring; }
            set {
                m_PositionSpring = value;
                if (Application.isPlaying && m_PositionSpring != null) { m_PositionSpring.Initialize(false, false); RefreshSprings(); }
            }
        }
        public Vector3 BasePositionOffset
        {
            get { return m_BasePositionOffset; }
            set {
                m_BasePositionOffset = value;
        ...
        public Spring RotationSpring { get { return m_RotationSpring; }
            set {
                ...
                if (Application.isPlaying && m_RotationSpring != null) { m_RotationSpring.Initialize(true, false); RefreshSprings(); }
            }
        }
        ...
        public Spring PivotPositionSpring { get { return m_PivotPositionSpring; }
            set {
                ...
                if (Application.isPlaying && m_PivotPositionSpring != null) { m_PivotPositionSpring.Initialize(false, false); RefreshSprings(); }
            }
        }
        ...
        public Spring PivotRotationSpring { get { return m_PivotRotationSpring; }
            set {
                ...
                if (Application.isPlaying && m_PivotRotationSpring != null) { m_PivotRotationSpring.Initialize(true, false); RefreshSprings(); }
            }
        }
        ...
        private Transform[] m_AdditionalControlObjectsTransforms;
        private Collider[] m_VisibleItemColliders;

        private bool m_IndependentItem = true;
        private bool m_ObjectAssignment;
        ...
        public CharacterItem CharacterItem { get { return m_CharacterItem; } }
        ...
        public override bool Initialize(GameObject character)
        {
            ...
            // Remote players do not need the first person arms/objects
            if (m_CharacterItem.NetworkInfo != null && !m_CharacterItem.NetworkInfo.IsLocalPlayer() && !m_CharacterItem.NetworkInfo.IsSpectator()) {
                if (m_VisibleItem != null) {
                    Destroy(m_VisibleItem);
                }
                if (m_Object != null) {
                    Destroy(m_Object);
                }
            ...
            if (m_Object == null) {
                var firstPersonObject = GetFirstPersonBaseObject(character);
                if (firstPersonObject == null) {
                    if (m_VisibleItem != null) { m_VisibleItem.SetActive(false); }
                    return false;
                }
                m_Object = firstPersonObject.gameObject;
                m_ObjectAssignment = true;

                // The VisibleItem should be positioned under the object.
                if (m_VisibleItem != null) {
                    var parent = GetSpawnParent(character, m_CharacterItem.SlotID, true); 
                    m_VisibleItem.transform.parent = parent;
                    m_VisibleItem.transform.localScale = m_LocalSpawnScale;
            ...
            if (m_VisibleItem != null) {
                m_VisibleItemColliders = m_VisibleItem.GetComponentsInChildren<Collider>(true);
            } else {
                m_VisibleItemColliders = Array.Empty<Collider>();
            }

            // The object needs to have a pivot transform.
            m_ObjectTransform = m_Object.transform;
            var firstPersonBaseObject = m_ObjectTransform.GetComponentInParent<FirstPersonBaseObject>(true);
            m_PivotTransform = firstPersonBaseObject.PivotTransform;
            m_FirstPersonObjects = GetFirstPersonObjects(m_Character);

            if (m_AdditionalControlObjectsTransforms == null) {
                m_AdditionalControlObjectsTransforms = new Transform[m_AdditionalControlObjects.Length];
            }
            for (int i = 0; i < m_AdditionalControlObjects.Length; ++i) {
                var controlObject = m_AdditionalControlObjects[i].GetObject(m_FirstPersonObjects.gameObject, true, true);
                if (controlObject != null) {
                    m_AdditionalControlObjectsTransforms[i] = controlObject.transform;
                } else {
                    Debug.LogError($"Error: Unable to find the control object with ID {m_AdditionalControlObjects[i].ID}.", m_FirstPersonObjects);
                }
            }
            ...
            m_PivotPositionSpring.Initialize(false, false);
            m_PivotRotationSpring.Initialize(true, false);
        ...
        /// <summary>
        /// Returns the matching FirstPersonBaseObject for the character model provided.
        /// </summary>
        /// <param name="character">The character model to get the object for.</param>
        /// <returns>The matching FirstPersonBaseObject (can be null).</returns>
        public FirstPersonBaseObject GetFirstPersonBaseObject(GameObject character)
        {
            var firstPersonObjects = GetFirstPersonObjects(character);
            // The character may not have a first person perspective setup.
            if (firstPersonObjects == null) {
                return null;
            }

            // A First Person Base Object ID can be specified if there are multiple FirstPersonBaseObjects and the item should be spawned under a particular
            // ItemID within that FirstPersonBaseObject.
            FirstPersonBaseObject firstPersonBaseObject = null;
            var foundMatch = false;
            var firstPersonObject = firstPersonObjects.transform.GetComponentsInChildren<FirstPersonBaseObject>(true);
            for (int i = 0; i < firstPersonObject.Length; ++i) {
                if (firstPersonObject[i].ID == m_FirstPersonBaseObjectID) {
                    foundMatch = true;
                    firstPersonBaseObject = firstPersonObject[i];
                    break;
                }
            }
        ...
        public FirstPersonObjects GetFirstPersonObjects(GameObject character)
        {
            var model = character;
            // First check if the character is the character or the model
            ...
            var firstPersonObjects = model.GetComponentInChildren<FirstPersonObjects>(true);
            if (firstPersonObjects != null) {
            ...
            for (int i = 0; i < firstPersonObjectsInCamera.Length; ++i) {
                if (firstPersonObjectsInCamera[i].CharacterModel == model) {
                    return firstPersonObjectsInCamera[i];
                }
            }
        ...
        {
            // Check for the slot within the FirstPersonObject.
            ...
            if (parentToItemSlotID) {
                ...
                var itemSlots = parent.GetComponentsInChildren<CharacterItemSlot>(true);
                for (int i = 0; i < itemSlots.Length; ++i) {
#if FIRST_PERSON_CONTROLLER
                    if (itemSlots[i].GetComponentInParent<FirstPersonBaseObject>(true) == null) {
                        continue;
                    }
#endif
            ...
            if (m_SpawnParent.TryGetObjectInChildren(parent.gameObject, out var spawnParent, true)) {
                if (spawnParent != null) {
                    return spawnParent;
                }
            }
        ...
        public override void ItemStarted()
        {
            ...
            var allItems = m_Character.GetComponentsInChildren<CharacterItem>(true);
            for (int i = 0; i < allItems.Length; ++i) {
                // Independent items are only a concept for first person objects.
                if (m_CharacterItem == allItems[i] || allItems[i].FirstPersonPerspectiveItem == null) {
                    continue;
                }
        ...
        public override void SetActive(bool active, bool hasItem, bool setIKTargets)
        {
            ...
            if (m_IndependentItem || (!IsActive() && active)
#if ULTIMATE_CHARACTER_CONTROLLER_VERSION_2_VR
                && !m_VRHandParent
#endif
                ) {
                ...
                m_PrevPlatformMovement = m_CharacterLocomotion.MovingPlatformMovement;
                m_PrevPlatformRotationMovement = m_CharacterLocomotion.MovingPlatformRotation;
            }
            if (m_VisibleItem != null) {
                m_VisibleItem.SetActive(active);
            }
            
            EnableItemColliders(active && m_CharacterLocomotion.FirstPersonPerspective);
        }
        ...
        {
            if (IsActive()) {
                EnableItemColliders(firstPersonPerspective);
            }
        }

        /// <summary>
        /// Enabled of disable the item colliders.
        /// </summary>
        /// <param name="enable">Enable or Disable the colliders?</param>
        protected virtual void EnableItemColliders(bool enable)
        {
            if(m_VisibleItemColliders == null){ return; }
            
            for (int i = 0; i < m_VisibleItemColliders.Length; i++) {
                m_VisibleItemColliders[i].enabled = enable;
            }
        }
        ...
        public override void Move(float horizontalMovement, float verticalMovement)
        {
            ...
            if (Utility.UnityEngineUtility.HasUpdatedObject(m_Object)) {
                return;
            }
            ...
            Utility.UnityEngineUtility.AddUpdatedObject(m_Object);
        ...
        private void UpdateSway()
        {
            ...
            var swayVelocity = m_CharacterLocomotion.LocalVelocity * m_PositionInputVelocityScale;
            ...
            swayVelocity *= m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale;

            // Rotate the item while changing vertical positions. The item will only rotate around the local z vector while moving down.
            var verticalSway = -swayVelocity.y * 0.005f * m_RotationVerticalSway;
            ...
            var platformPositionalDiff = m_CharacterTransform.InverseTransformDirection(m_PrevPlatformMovement - m_CharacterLocomotion.MovingPlatformMovement);
            ...
            positionForce += strafeValue;
            var platformEulerDiff = MathUtility.InverseTransformQuaternion(m_CharacterTransform.rotation, (m_CharacterLocomotion.MovingPlatformRotation * Quaternion.Inverse(m_PrevPlatformRotationMovement))).eulerAngles;
        ...
        private void UpdateBob()
        {
            ...
            m_CurrentBobPositionalValue.x = Mathf.Cos(m_BobPositionalRate.x * 10.0f * m_CharacterLocomotion.TimeScale * TimeUtility.Time) * currentPositionalBobAmplitude.x;
            m_CurrentBobPositionalValue.y = Mathf.Cos(m_BobPositionalRate.y * 10.0f * m_CharacterLocomotion.TimeScale * TimeUtility.Time) * currentPositionalBobAmplitude.y;
            m_CurrentBobPositionalValue.z = Mathf.Cos(m_BobPositionalRate.z * 10.0f * m_CharacterLocomotion.TimeScale * TimeUtility.Time) * currentPositionalBobAmplitude.z;

            var currentRotationalBobValue = (bobSpeed * (m_BobRotationalAmplitude * -0.0001f));
            currentRotationalBobValue.x = Mathf.Cos(m_BobRotationalRate.x * 10.0f * m_CharacterLocomotion.TimeScale * TimeUtility.Time) * currentRotationalBobValue.x;
            currentRotationalBobValue.y = Mathf.Cos(m_BobRotationalRate.y * 10.0f * m_CharacterLocomotion.TimeScale * TimeUtility.Time) * currentRotationalBobValue.y;
            currentRotationalBobValue.z = Mathf.Cos(m_BobRotationalRate.z * 10.0f * m_CharacterLocomotion.TimeScale * TimeUtility.Time) * currentRotationalBobValue.z;
        ...
        {
            if (m_StepMinVelocity <= 0 || (m_BobRequireGroundContact && !m_CharacterLocomotion.Grounded) || m_CharacterLocomotion.Velocity.sqrMagnitude < m_StepMinVelocity) {
                return;
            }
            ...
            if (elevating && !m_BobWasElevating) {
                ...
                if (Mathf.Cos(m_CharacterLocomotion.TimeScale * TimeUtility.Time * (m_BobPositionalRate.x * 5)) > 0) {
                ...
                m_PivotPositionSpring.AddForce(m_StepForceScale * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale * positionForce, m_StepSoftness);
                m_PivotRotationSpring.AddForce(m_StepForceScale * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale * rotationForce, m_StepSoftness);
            }
        ...
        private void ApplyMovement()
        {
            ...
            for (int i = 0; i < m_AdditionalControlObjectsTransforms.Length; ++i) {
                m_AdditionalControlObjectsTransforms[i].SetPositionAndRotation(m_ObjectTransform.position, m_ObjectTransform.rotation);
            }
        }
        ...
        private void OnAddPivotForce(int slotID, Vector3 positionalForce, Vector3 rotationalForce, bool globalForce)
        {
            ...
            if (slotID != -1 && slotID != m_CharacterItem.SlotID) {
                return;
            }
        ...
        private void RefreshSprings()
        {
            ...
            // Set the rest state.
            m_PositionSpring.RestValue = m_BasePositionOffset + (m_UseSpringExitOffset ? m_PositionExitOffset : m_PositionOffset);
        ...
        /// <param name="resetSpring">Should the springs be reset?</param>
        private void SnapSprings(bool fromEquip, bool resetSpring = true)
        {
            m_PositionSpring.RestValue = m_BasePositionOffset + (m_UseSpringExitOffset ? m_PositionExitOffset : m_PositionOffset);
            m_RotationSpring.RestValue = m_UseSpringExitOffset ? m_RotationExitOffset : m_RotationOffset;
            if (resetSpring) {
            ...
            if ((!fromEquip && (Utility.UnityEngineUtility.HasUpdatedObject(m_Object) || !IsActive())) || !m_CharacterItem.DominantItem) {
                return;
            }
            // The object hasn't been updated this frame. The updated object set will be cleared after the frame is complete.
            Utility.UnityEngineUtility.AddUpdatedObject(m_Object, true);
        ...
        {
            base.StartEquip(immediateEquip);

            // The FirstPersonObjects component should first be aware of the equip so it can correctly activate the base object.
            EventHandler.ExecuteEvent(m_Character, "OnFirstPersonPerspectiveItemStartEquip", m_CharacterItem);

            // Non-dominant items cannot control spring location.
            if (!m_CharacterItem.DominantItem) {
                return;
            }

            // If the character is scaled then the local scale would have changed as the item is equipped and unequipped.
            if (m_VisibleItem != null) {
                m_VisibleItem.transform.localScale = m_LocalSpawnScale;
            }
        ...
        public override void Unequip()
        {
            ...
            EventHandler.ExecuteEvent(m_Character, "OnFirstPersonPerspectiveItemUnequip", m_CharacterItem);
        ...
        {
            if (m_CharacterItem == characterItem || characterItem.FirstPersonPerspectiveItem == null) {
                return;
            }

            if (m_Object == characterItem.FirstPersonPerspectiveItem.Object) {
        ...
        {
            SnapSprings(false, snapAnimator);
        }
        ...
        public override void ResetInitialization()
        {
            ...
            if (m_ObjectAssignment) {
                m_Object = null;
            }
            if (m_VisibleItem != null) {
                m_VisibleItem.transform.SetParent(m_CharacterItem.transform);
            }
        ...
        public override void OnCharacterSwitchModels(GameObject activeModel)
        {
            ...
            var firstPersonBaseObject = GetFirstPersonBaseObject(activeModel);
            if (firstPersonBaseObject == null) {
                return;
            }
            m_Object = firstPersonBaseObject.gameObject;
            m_ObjectTransform = m_Object.transform;
            m_PivotTransform = firstPersonBaseObject.PivotTransform;

            // The VisibleItem should be positioned under the object.
            if (m_VisibleItem != null) {
                var parent = GetSpawnParent(activeModel, m_CharacterItem.SlotID, true);
                m_VisibleItem.transform.parent = parent;
                m_VisibleItem.transform.localPosition = m_LocalSpawnPosition;
                m_VisibleItem.transform.localRotation = Quaternion.Euler(m_LocalSpawnRotation);
                m_VisibleItem.transform.localScale = m_LocalSpawnScale;
            ...
            // The FirstPersonObjects component needs to be aware of the new Object model.
            m_FirstPersonObjects = GetFirstPersonObjects(activeModel);
            
            // Get the new AdditionalControlObjects.
            for (int i = 0; i < m_AdditionalControlObjects.Length; ++i) {
                var controlObject = m_AdditionalControlObjects[i].GetObject(m_FirstPersonObjects.gameObject, true, true);
                if (controlObject != null) {
                    m_AdditionalControlObjectsTransforms[i] = controlObject.transform;
