...
namespace Opsive.UltimateCharacterController.FirstPersonController.Character
{
    ...
    public class FirstPersonObjects : StateBehavior
    {
        ...
        private void LateUpdate()
        {
            ...
            m_Transform.rotation = Quaternion.Slerp(m_Transform.rotation, rotation, m_RotationSpeed * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale * TimeUtility.DeltaTime);
            ...
            if (m_IgnorePositionalLookOffset && m_CameraController.ActiveViewType is FirstPersonController.Camera.ViewTypes.FirstPerson) {
                ...
                m_Transform.position = Vector3.MoveTowards(m_Transform.position, targetPosition, m_MoveSpeed * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale * TimeUtility.DeltaTime);
            } else if (m_Transform.localPosition != Vector3.zero) {
                m_Transform.localPosition = Vector3.MoveTowards(m_Transform.localPosition, Vector3.zero, m_MoveSpeed * m_CharacterLocomotion.TimeScale * TimeUtility.TimeScale * TimeUtility.DeltaTime);
            }
        }
