...
namespace Opsive.UltimateCharacterController.Inventory
{
    ...
    public abstract class ItemSetManagerBase : MonoBehaviour
    {
        ...
        public virtual void Initialize(bool force)
        {
            ...
            EventHandler.RegisterEvent(m_GameObject, "OnInventoryLoadDefaultLoadoutComplete",
                OnInventoryLoadDefaultLoadoutComplete);
        }
        ...
        public bool TryEquipItemSet(string itemSetName, int groupIndex, bool forceEquipUnequip,
            bool immediateEquipUnequip)
        {
        ...
        /// <param name="immediateEquipUnequip">Immediately equip the item?</param>
        /// <returns>Return true if an item set exist with that name.</returns>
        public bool TryEquipItemSet(int itemSetIndex, int groupIndex, bool forceEquipUnequip,
            bool immediateEquipUnequip)
        {
        ...
        /// <param name="forceEquipUnequip">Force equip the item?</param>
        /// <param name="immediateEquipUnequip">Immediately equip the item?</param>
        public void EquipUnequipItem(IItemIdentifier item, bool equip, int groupIndex, bool forceEquipUnequip,
            bool immediateEquipUnequip)
        {
            ...
            if (itemSet.Active == equip) {
                return;
            }

            if (itemSet.EquipUnequip == null) {
                Debug.LogError(
                    $"The Equip Unequip Ability for the ItemSet '{itemSet.State}' of group '{itemSet.ItemSetGroup.CategoryName}' is null");
                return;
            }
        ...
        public void UnEquipItem(IItemIdentifier item, int groupIndex, bool forceEquipUnequip,
            bool immediateEquipUnequip)
        {
        ...
        public int GetTargetItemSetIndex(int groupIndex, int allowedMask)
        {
            ...
            if (itemSetIndex != -1 && prevItemSetIndex != itemSetIndex &&
                IsItemSetValid(groupIndex, prevItemSetIndex, false, allowedMask)) {
                for (int i = 0; i < itemSetGroup.ItemSetList[prevItemSetIndex].ItemIdentifiers.Length; ++i) {
                    ...
                    if (itemSetGroup.ItemSetList[itemSetIndex].ItemIdentifiers[i] == prevSlots[i] &&
                        prevSlots[i] != null) {
            ...
            } while (itemSetIndex == prevItemSetIndex || itemSetIndex == itemSetGroup.DefaultItemSetIndex ||
                     !IsItemSetValid(groupIndex, itemSetIndex, false, allowedMask));
        ...
        {
            if (groupIndex == -1 || groupIndex >= m_ItemSetGroups.Length || slot == -1 ||
                slot >= m_Inventory.SlotCount) {
                return null;
            }
        ...
        public IItemIdentifier GetNextItemIdentifier(int slot, out int groupIndex)
        {
            ...
            for (int i = 0; i < m_ItemSetGroups.Length; ++i) {
                var itemSetGroup = m_ItemSetGroups[i];
                var index = itemSetGroup.NextItemSetIndex != -1
                    ? itemSetGroup.NextItemSetIndex
                    : itemSetGroup.ActiveItemSetIndex;
                if (index == -1) {
                    continue;
                }
        ...
        protected virtual void OnDestroy()
        {
            ...
            if (m_GameObject != null) {
                ...
                EventHandler.UnregisterEvent(m_GameObject, "OnInventoryLoadDefaultLoadoutComplete",
                    OnInventoryLoadDefaultLoadoutComplete);
            }
        }
        ...
        public void UnEquipAllItems(bool forceEquipUnequip, bool immediateEquipUnequip)
        {
            for (int i = 0; i < m_ItemSetGroups.Length; i++) {
                ...
                if (activeItemSet.EquipUnequip == null) {
                    Debug.LogError(
                        $"The Equip Unequip Ability for the ItemSet '{activeItemSet.State}' of group '{activeItemSet.ItemSetGroup.CategoryName}' is null");
                    return;
                }
        ...
        /// <summary>
        /// Finds the closest matching valid item set based on shared items and index matches.
        /// </summary>
        /// <param name="groupIndex">The index of the item set group to check.</param>
        /// <param name="itemSetIndex">The index of the item set to match against.</param>
        /// <param name="useDefaultIfNoMatch">If true, the default item set will be returned if no match is found.</param>
        /// <param name="matchThisItemSetIfValid">If true, return the item set index provided if it is valid.</param>
        /// <returns>The index of the closest matching valid item set, or -1 if no match is found.</returns>
        public int FindClosestMatchingValidItemSet(int groupIndex, int itemSetIndex, bool useDefaultIfNoMatch,
            bool matchThisItemSetIfValid)
        {
            if (groupIndex < 0 || groupIndex >= m_ItemSetGroups.Length) {
                if (useDefaultIfNoMatch) {
                    return GetDefaultItemSetIndex(groupIndex);
                }
            ...
            var itemSetGroup = m_ItemSetGroups[groupIndex];
            return itemSetGroup.FindClosestMatchingValidItemSet(itemSetIndex, useDefaultIfNoMatch,
                matchThisItemSetIfValid);
        }

        /// <summary>
        /// Determines if the specified character item is part of a valid item set.
        /// </summary>
        /// <param name="characterItem">The character item to check.</param>
        /// <returns>True if the character item is part of a valid item set; otherwise, false.</returns>
        public bool IsItemPartOfValidItemSet(CharacterItem characterItem)
        {
            foreach (var itemSetGroup in m_ItemSetGroups) {
                if (itemSetGroup.IsInitialized == false) {
                    continue;
                }

                foreach (var itemSet in itemSetGroup.ItemSetList) {
                    // Check if the item identifier matches the character item's slot ID and identifier.
                    if (itemSet.ItemIdentifiers[characterItem.SlotID] == characterItem.ItemIdentifier) {
                        if (itemSet.IsValid) {
                            return true;
                        }
                    }
            ...
            // Return false if no valid item set is found.
            return false;
        }

        /// <summary>
        /// Determines if the specified item identifier is part of a valid item set.
        /// </summary>
        /// <param name="itemIdentifier">The item identifier to check.</param>
        /// <returns>True if the item identifier is part of a valid item set; otherwise, false.</returns>
        public bool IsItemPartOfValidItemSet(IItemIdentifier itemIdentifier)
        {
            foreach (var itemSetGroup in m_ItemSetGroups) {
                if (itemSetGroup.IsInitialized == false) {
                    continue;
